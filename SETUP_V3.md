...
1. Im Supabase SQL Editor `migration_v3.sql` komplett ausführen.
...
4. In `bootstrap_first_admin.sql` `DEINE-EMAIL@BEISPIEL.DE` durch genau diese E-Mail ersetzen und das Script im SQL Editor ausführen.
...
## Datenerhalt
`migration_v3.sql` kopiert den bisherigen Datensatz aus `volleyball_trainer_state` in `vt_team_states`. Die alte Tabelle bleibt bestehen und kann als zusätzlicher Rückfallpunkt genutzt werden.
