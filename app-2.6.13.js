(() => {
"use strict";
const VERSION="2.6.13",DATA_SCHEMA=3,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function ruleIssues(){
  ...
  steps.forEach((step,i)=>{
    const a=step.action||{};
    if(!a.type||a.type==="block"||!a.actorId)return;
    const team=playerTeam(a.actorId);if(!team)return;
    if(team!==lastRegularTeam){lastRegularTeam=team;count=1}else count++;
    if(count>3){const teamName=team==="own"?"eigene Mannschaft":"Gegner";add("error",i,`${step.name}: ${teamName} hat ${count} Ballkontakte in Folge. Erlaubt sind maximal 3; ein Block zählt nicht als Ballkontakt.`)}
  });
  return issues;
}
