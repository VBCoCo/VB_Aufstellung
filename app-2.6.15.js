(() => {
"use strict";
const VERSION="2.6.15",DATA_SCHEMA=3,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
const ids=["infoButton","infoButtonBottom","editButton","rotationSelect","situationNameInput","addSituation","deleteSituation","stepNumber","stepTotal","prevStep","playButton","nextStep","stepEditPanel","stepNameWrap","stepNamePreset","stepNameInput","newStepNaming","saveStep","addStep","deleteStep","editPanel","situationSelect","currentStepTitle","court","validationLayer","movementLayer","ballPathLayer","playerLayer","ballObject","tapNotice","status","infoDialog","closeInfo","closeInfoBottom","infoSituation","lineupToggle","lineupChevron","lineupEditor","lineupGrid","liberoToggle","syncBadge","jsBuildBadge","passwordDialog","closePassword","cancelPassword","confirmPassword","passwordInput","passwordStatus","migrationPanel","dataSourceStatus","migrateLocalButton","migrationHint","viewToggle","view2d","view25d","positionInfoToggle","court25d","court25Floor","movement25Layer","ballPath25Layer","action25Layer","player25Layer","ball25Object","actionPanel","actionType","actionActor","actionTechniqueWrap","actionTechnique","actionHelperWrap","actionHelper","attackBlockWrap","attackBlocker1","attackBlocker2","snapBallToActor","clearAction","actionHint","actionSummary","actionLayer","ruleCheck","ruleCheckToggle","ruleCheckSummary","ruleCheckChevron","ruleCheckDetails"];
const e=Object.fromEntries(ids.map(id=>[id,document.getElementById(id)]));
if(e.jsBuildBadge)e.jsBuildBadge.textContent=`JS ${VERSION} geladen`;
...
const initialStep=r=>{const positions={};ownRoster.forEach(p=>positions[p.id]={...ownSlots[rot(p.base,r)]});opponentRoster.forEach(p=>positions[p.id]={...opponentSlots[p.base]});return{name:"Grundposition",situation:"serveReceive",positions,ball:{x:450,y:650},action:{type:"",actorId:"",technique:"",helperId:"",blocker1Id:"",blocker2Id:""}}};
const defaultSituationName=i=>i===0?"Grundaufstellung":`Grundaufstellung +${i}`;
const fresh=()=>({schemaVersion:DATA_SCHEMA,rotation:0,step:0,teamConfig:{roles:{...defaultRoles},libero:false},rotations:Array.from({length:6},(_,r)=>({name:defaultSituationName(r),rotationOffset:r,steps:[initialStep(r)]}))});
...
function migrate(){
  let changed=false;
  if(!state||!Array.isArray(state.rotations)||!state.rotations.length){state=fresh();return true}
  if(Number(state.schemaVersion)!==DATA_SCHEMA){state.schemaVersion=DATA_SCHEMA;changed=true}
  if(!state.teamConfig){state.teamConfig={roles:{...defaultRoles},libero:false};changed=true}
  const mergedRoles={...defaultRoles,...(state.teamConfig.roles||{})};if(JSON.stringify(mergedRoles)!==JSON.stringify(state.teamConfig.roles||{})){state.teamConfig.roles=mergedRoles;changed=true}else state.teamConfig.roles=mergedRoles;
  state.teamConfig.libero=Boolean(state.teamConfig.libero);
  state.rotations.forEach((rotation,r)=>{
    if(!rotation.name){rotation.name=defaultSituationName(r);changed=true}
    const offset=Number.isFinite(Number(rotation.rotationOffset))?Number(rotation.rotationOffset)%6:r%6;if(rotation.rotationOffset!==offset){rotation.rotationOffset=offset;changed=true}
    if(!Array.isArray(rotation.steps)||!rotation.steps.length){rotation.steps=[initialStep(rotation.rotationOffset)];changed=true}
    rotation.steps.forEach(step=>{
      ...
      if(!step.situation){step.situation="serveReceive";changed=true}
      if(!step.name){step.name="Grundposition";changed=true}
      const old=step.action||{};
      const type=old.type||"",actorId=old.actorId||"",helperId=old.helperId||"",blocker1Id=old.blocker1Id||"",blocker2Id=old.blocker2Id||"";
      let technique=old.technique||"";
      if(technique==="oben")technique="upper";
      if(technique==="unten")technique="lower";
      // Alte Daten vor Einführung der Technikauswahl hatten bei Annahme/Zuspiel/Abwehr keinen Technikwert.
      // Diese Kontakte waren damals visuell als oberer Standardkontakt gedacht, daher automatisch auf "upper" migrieren.
      if(["receive","set","defense"].includes(type)&&!["upper","lower"].includes(technique))technique="upper";
      if(!["receive","set","defense"].includes(type))technique="";
      const normalized={type,actorId,technique,helperId:type==="block"?helperId:"",blocker1Id:type==="attack"?blocker1Id:"",blocker2Id:type==="attack"?blocker2Id:""};
      if(JSON.stringify(normalized)!==JSON.stringify({type:old.type||"",actorId:old.actorId||"",technique:old.technique||"",helperId:old.helperId||"",blocker1Id:old.blocker1Id||"",blocker2Id:old.blocker2Id||""}))changed=true;
      step.action=normalized;
    });
  });
  const rotation=Math.max(0,Math.min(state.rotations.length-1,Number(state.rotation)||0));if(rotation!==state.rotation){state.rotation=rotation;changed=true}else state.rotation=rotation;
  const step=Math.max(0,Math.min(state.rotations[state.rotation].steps.length-1,Number(state.step)||0));if(step!==state.step){state.step=step;changed=true}else state.step=step;
  return changed;
}
const localMigrationApplied=migrate();if(localMigrationApplied)localStorage.setItem(KEY,JSON.stringify(state));
let editing=false,selected=null,dragging=null,playing=false,animations=[],editorPassword=null,lineupOpen=false,committedState=structuredClone(state),dirty=false,namingNewStep=false;
...
function renderActionEditor(){
  if(!e.actionType)return;const a=actionData();populatePlayerSelect(e.actionActor);populatePlayerSelect(e.actionHelper,{label:"– kein zweiter Spieler –"});populateBlockerSelect(e.attackBlocker1,a.actorId);populateBlockerSelect(e.attackBlocker2,a.actorId);
  e.actionType.value=a.type;e.actionActor.value=a.actorId;e.actionTechniqueWrap.classList.toggle("hidden",!supportsTechnique(a.type));e.actionTechnique.value=a.technique||"upper";e.actionHelper.value=a.helperId;e.actionHelperWrap.classList.toggle("hidden",a.type!=="block");e.attackBlockWrap.classList.toggle("hidden",a.type!=="attack");e.attackBlocker1.value=a.blocker1Id;e.attackBlocker2.value=a.blocker2Id;
  ...
  let txt=a.type?`${actionNames[a.type]||a.type}`:"Kein Ballkontakt definiert";if(supportsTechnique(a.type)&&a.technique)txt+=` (${techniqueName(a.technique)})`;if(actor)txt+=` · ${playerLabel(actor)}`;if(helper)txt+=` + ${playerLabel(helper)}`;if(a.type==="attack"&&b1){txt+=` · Blockversuch: ${playerLabel(b1)}`;if(b2)txt+=` + ${playerLabel(b2)}`}
  const next=rd().steps[state.step+1];e.actionHint.textContent=a.type?(next?`Ballkontakt in Schritt ${state.step+1}. Beim Übergang zu Schritt ${state.step+2} fliegt der Ball zum dort gespeicherten Kontaktpunkt; alle Spieler bewegen sich gleichzeitig.`:"Letzter Schritt: Kontakt gespeichert. Für einen weiteren Übergang einen neuen Schritt anlegen."):"Optional: Kontaktart und ausführenden Spieler festlegen. Der Ballflug ergibt sich automatisch aus dem nächsten Schritt.";
  e.actionSummary.textContent=a.type?txt:" ";e.actionSummary.classList.remove("hidden");
}
function render25(){
...
function ruleIssues(){
  ...
  steps.forEach((step,i)=>{
    ...
    if(a.type&&!actor)add("error",i,`${step.name}: Ballkontakt „${actionNames[a.type]||a.type}“ hat keinen Kontaktspieler.`);
    ...
    if(a.type&&a.type!=="block"&&actor&&actorPos&&step.ball&&distance(actorPos,step.ball)>65){
...
function renderRuleCheck(){
  ...
  const issues=ruleIssues(),errors=issues.filter(x=>x.severity==="error").length,warnings=issues.filter(x=>x.severity==="warning").length;
  const visible=editing&&issues.length>0;
  e.ruleCheck.classList.toggle("hidden",!visible);
  if(!visible){e.ruleCheckDetails.classList.add("hidden");e.ruleCheckToggle.setAttribute("aria-expanded","false");e.ruleCheckChevron.textContent="▾";return}
  e.ruleCheck.classList.remove("rule-ok","rule-warning","rule-error");e.ruleCheck.classList.add(errors?"rule-error":"rule-warning");
...
function render(){
  renderSituationOptions();e.stepNumber.textContent=state.step+1;e.stepTotal.textContent=rd().steps.length;e.stepNameInput.value=sd().name;e.stepNamePreset.value=["Aufschlag","Annahme","Zuspiel","Angriff","Angriffssicherung","Block","Doppelblock","Abwehr"].includes(sd().name)?sd().name:"";e.currentStepTitle.textContent=sd().name;e.situationSelect.value=sd().situation;e.liberoToggle.checked=state.teamConfig.libero;document.body.classList.toggle("editing-mode",editing);updateMigrationUI();e.addStep.classList.toggle("hidden",!editing);e.saveStep.classList.toggle("hidden",!editing||!dirty);e.newStepNaming.classList.toggle("hidden",!editing||!namingNewStep);renderActionEditor();paths();const bad=validate();
...
function edit(v){stop();clearVisualAnimations();if(v&&!editing){state=structuredClone(committedState);dirty=false}if(!v&&editing&&dirty){state=structuredClone(committedState);migrate();dirty=false}editing=v;selected=dragging=null;if(!v)namingNewStep=false;e.editButton.textContent=v?"✓ Fertig":"✎ Bearbeiten";[e.stepEditPanel,e.editPanel].forEach(x=>x.classList.toggle("hidden",!v));e.tapNotice.classList.add("hidden");if(!v){lineupOpen=false;e.lineupEditor.classList.add("hidden");e.lineupChevron.textContent="▾"}render()}
...
function playerPoint(p,player){const a=actionData();if(a.type==="serve"&&a.actorId===player.id){if(player.team==="own")return{x:clamp(p.x,124,576),y:clamp(p.y,855,888)};return{x:clamp(p.x,124,576),y:clamp(p.y,12,45)}}if(sd().situation==="serveReceive"){const yMin=player.team==="own"?479:79,yMax=player.team==="own"?821:421;return{x:clamp(p.x,124,576),y:clamp(p.y,yMin,yMax)}}return{x:clamp(p.x,32,668),y:clamp(p.y,32,868)}}
const cb=p=>({x:clamp(p.x,25,675),y:clamp(p.y,25,875)});
function keepServeReceiveInside(){if(sd().situation!=="serveReceive")return;const a=actionData();allPlayers.forEach(player=>{if(a.type==="serve"&&a.actorId===player.id)return;sd().positions[player.id]=playerPoint(sd().positions[player.id],player)})}
e.court.addEventListener("pointerdown",ev=>{if(!editing||playing)return;ev.preventDefault();const pe=ev.target.closest("[data-id]"),be=ev.target.closest("#ballObject");if(selected){if(selected.type==="player"&&pe?.dataset.id===selected.id){selected=null;e.tapNotice.classList.add("hidden");render();return}const p=point(ev);if(selected.type==="player"){const player=allPlayers.find(x=>x.id===selected.id);sd().positions[selected.id]=playerPoint(p,player);if(actionData().actorId===selected.id)syncContactBall();dirty=true}else{sd().ball=cb(p);dirty=true}selected=null;e.tapNotice.classList.add("hidden");render();return}if(pe){selected={type:"player",id:pe.dataset.id};e.tapNotice.classList.remove("hidden");render();return}if(be){if(actionData().actorId){e.status.textContent="Der Ball ist in diesem Schritt an den Kontaktspieler gekoppelt. Zum freien Verschieben zuerst den Kontakt löschen.";return}selected={type:"ball"};e.tapNotice.classList.remove("hidden");render()}} ,{passive:false});

function goStep(target){target=Math.max(0,Math.min(rd().steps.length-1,target));if(target===state.step)return;if(editing){clearVisualAnimations();state.step=target;selected=dragging=null;namingNewStep=false;render();return}animate(target)}
...
e.editButton.addEventListener("click",requestEdit);e.rotationSelect.addEventListener("change",x=>{state.rotation=Number(x.target.value);state.step=0;namingNewStep=false;buildLineupEditor();render()});e.situationNameInput.addEventListener("input",()=>{rd().name=e.situationNameInput.value.trim()||defaultSituationName(state.rotation);dirty=true;render()});e.stepNamePreset.addEventListener("change",()=>{if(e.stepNamePreset.value){e.stepNameInput.value=e.stepNamePreset.value;dirty=true}});e.stepNameInput.addEventListener("input",()=>{e.stepNamePreset.value=["Aufschlag","Annahme","Zuspiel","Angriff","Angriffssicherung","Block","Doppelblock","Abwehr"].includes(e.stepNameInput.value.trim())?e.stepNameInput.value.trim():"";dirty=true});e.addSituation.addEventListener("click",()=>{const copy=structuredClone(rd());copy.name=`Neue Spielsituation ${state.rotations.length+1}`;state.rotations.splice(state.rotation+1,0,copy);state.rotation++;state.step=0;dirty=true;render()});e.deleteSituation.addEventListener("click",()=>{if(state.rotations.length===1){e.status.textContent="Die einzige Spielsituation kann nicht gelöscht werden.";return}state.rotations.splice(state.rotation,1);state.rotation=Math.max(0,state.rotation-1);state.step=0;dirty=true;render()});e.situationSelect.addEventListener("change",x=>{sd().situation=x.target.value;keepServeReceiveInside();dirty=true;render()});
e.saveStep.addEventListener("click",()=>{if(namingNewStep)sd().name=e.stepNameInput.value.trim()||`Schritt ${state.step+1}`;keepServeReceiveInside();save("Änderungen gespeichert.");namingNewStep=false;render()});
e.addStep.addEventListener("click",()=>{keepServeReceiveInside();const s=sd();rd().steps.splice(state.step+1,0,{name:`Schritt ${state.step+2}`,situation:s.situation,positions:structuredClone(s.positions),ball:structuredClone(s.ball),action:{type:"",actorId:"",technique:"",helperId:"",blocker1Id:"",blocker2Id:""}});state.step++;namingNewStep=true;dirty=true;render();setTimeout(()=>e.stepNamePreset.focus(),0)});
e.deleteStep.addEventListener("click",()=>{if(rd().steps.length===1){e.status.textContent="Der einzige Schritt kann nicht gelöscht werden.";return}rd().steps.splice(state.step,1);state.step=Math.max(0,state.step-1);namingNewStep=false;dirty=true;render()});

e.actionType.addEventListener("change",()=>{sd().action={...actionData(),type:e.actionType.value};if(supportsTechnique(sd().action.type)&&!sd().action.technique)sd().action.technique="upper";if(!supportsTechnique(sd().action.type))sd().action.technique="";if(sd().action.type!=="block")sd().action.helperId="";if(sd().action.type!=="attack"){sd().action.blocker1Id="";sd().action.blocker2Id=""}ensureServeActorOutside();dirty=true;render()});
e.actionActor.addEventListener("change",()=>{sd().action={...actionData(),actorId:e.actionActor.value,blocker1Id:"",blocker2Id:""};ensureServeActorOutside();syncContactBall();dirty=true;render()});
...
e.clearAction.addEventListener("click",()=>{sd().action={type:"",actorId:"",technique:"",helperId:"",blocker1Id:"",blocker2Id:""};dirty=true;render()});
