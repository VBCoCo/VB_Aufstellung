(() => {
"use strict";
const VERSION="2.6.16",DATA_SCHEMA=3,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function renderRuleCheck(){
  if(!e.ruleCheck)return;const issues=ruleIssues(),errors=issues.filter(x=>x.severity==="error").length,warnings=issues.filter(x=>x.severity==="warning").length;
  e.ruleCheck.classList.remove("rule-ok","rule-warning","rule-error");e.ruleCheck.classList.add(errors?"rule-error":warnings?"rule-warning":"rule-ok");
  e.ruleCheckSummary.textContent=errors?`⛔ ${errors} Fehler${warnings?` · ${warnings} Warnung${warnings===1?"":"en"}`:""}`:warnings?`⚠ ${warnings} Warnung${warnings===1?"":"en"}`:"✓ Keine Regelprobleme";
  e.ruleCheckDetails.innerHTML="";
  if(issues.length){const ul=document.createElement("ul");issues.forEach(issue=>{const li=document.createElement("li");li.className=`rule-item-${issue.severity}`;li.textContent=`Schritt ${issue.stepIndex+1}: ${issue.message}`;ul.appendChild(li)});e.ruleCheckDetails.appendChild(ul)}else{const p=document.createElement("div");p.textContent="Für die aktuell definierten Aktionen wurden keine Regelprobleme gefunden.";e.ruleCheckDetails.appendChild(p)}
}
