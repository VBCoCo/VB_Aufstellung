(() => {
"use strict";
const VERSION="2.6.18",DATA_SCHEMA=3,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
e.editButton.addEventListener("click",requestEdit);e.rotationSelect.addEventListener("change",x=>{state.rotation=Number(x.target.value);state.step=0;namingNewStep=false;buildLineupEditor();render()});e.situationNameInput.addEventListener("input",()=>{});e.stepNamePreset.addEventListener("change",()=>{if(e.stepNamePreset.value){e.stepNameInput.value=e.stepNamePreset.value;dirty=true}});e.stepNameInput.addEventListener("input",()=>{e.stepNamePreset.value=["Aufschlag","Annahme","Zuspiel","Angriff","Angriffssicherung","Block","Doppelblock","Abwehr"].includes(e.stepNameInput.value.trim())?e.stepNameInput.value.trim():"";dirty=true});e.saveSituation.addEventListener("click",()=>{});e.addSituation.addEventListener("click",()=>{if(!editing)return;const suggested=`Neue Spielsituation ${state.rotations.length+1}`;const entered=window.prompt("Name der neuen Spielsituation:",suggested);if(entered===null)return;const name=entered.trim()||suggested;const copy=structuredClone(rd());copy.name=name;state.rotations.splice(state.rotation+1,0,copy);state.rotation++;state.step=0;namingNewStep=false;dirty=true;buildLineupEditor();render()});e.deleteSituation.addEventListener("click",()=>{if(!editing)return;if(state.rotations.length===1){e.status.textContent="Die einzige Spielsituation kann nicht gelöscht werden.";return}const name=rd().name||defaultSituationName(state.rotation);if(!window.confirm(`Spielsituation „${name}“ wirklich löschen?\n\nDabei werden alle Schritte dieser Spielsituation gelöscht.`))return;state.rotations.splice(state.rotation,1);state.rotation=Math.max(0,state.rotation-1);state.step=0;namingNewStep=false;dirty=true;buildLineupEditor();render()});e.situationSelect.addEventListener("change",x=>{sd().situation=x.target.value;keepServeReceiveInside();dirty=true;render()});
