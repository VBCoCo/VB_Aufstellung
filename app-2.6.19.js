(() => {
"use strict";
const VERSION="2.6.19",DATA_SCHEMA=4,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
const initialStep=r=>{const positions={};ownRoster.forEach(p=>positions[p.id]={...ownSlots[rot(p.base,r)]});opponentRoster.forEach(p=>positions[p.id]={...opponentSlots[p.base]});return{name:"Grundposition",situation:"serveReceive",positions,ball:{x:450,y:650},action:{type:"",actorId:"",technique:"",helperId:"",blocker1Id:"",blocker2Id:"",outcome:"",reason:""}}};
...
function migrate(){
  ...
  state.rotations.forEach((rotation,r)=>{
    ...
    rotation.steps.forEach(step=>{
      ...
      const type=old.type||"",actorId=old.actorId||"",helperId=old.helperId||"",blocker1Id=old.blocker1Id||"",blocker2Id=old.blocker2Id||"",outcome=old.outcome||"",reason=old.reason||"";
...
let editing=false,selected=null,dragging=null,playing=false,animations=[],editorPassword=null,lineupOpen=false,committedState=structuredClone(state),dirty=false,namingNewStep=false,actionMenuOpen=false;
...
function edit(v){stop();clearVisualAnimations();if(v&&!editing){state=structuredClone(committedState);dirty=false}if(!v&&editing&&dirty){state=structuredClone(committedState);migrate();dirty=false}editing=v;selected=dragging=null;if(!v){namingNewStep=false;actionMenuOpen=false;}e.editButton.textContent=v?"✓ Fertig":"✎ Bearbeiten";[e.stepEditPanel,e.editPanel].forEach(x=>x.classList.toggle("hidden",!v));e.tapNotice.classList.add("hidden");if(!v){lineupOpen=false;e.lineupEditor.classList.add("hidden");e.lineupChevron.textContent="▾"}render()}
...
e.actionType.addEventListener("change",()=>{const type=e.actionType.value;sd().action={...actionData(),type};if(type==="point"){sd().action.actorId="";sd().action.technique="";sd().action.helperId="";sd().action.blocker1Id="";sd().action.blocker2Id="";sd().action.outcome=sd().action.outcome||"own";sd().action.reason=sd().action.reason||"ground"}else{sd().action.outcome="";sd().action.reason="";if(supportsTechnique(type)&&!sd().action.technique)sd().action.technique="upper";if(!supportsTechnique(type))sd().action.technique="";if(type!=="block")sd().action.helperId="";if(type!=="attack"){sd().action.blocker1Id="";sd().action.blocker2Id=""}}ensureServeActorOutside();dirty=true;render()});
