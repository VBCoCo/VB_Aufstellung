(() => {
"use strict";
const VERSION="2.7.1",DATA_SCHEMA=5,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
const ids=["teamConfigClose","teamConfigPanel","teamConfigToggle","deleteTeam","addTeam","teamSelect","infoButton","infoButtonBottom","editButton","rotationSelect","situationNameInput","saveSituation","addSituation","deleteSituation","stepNumber","stepTotal","prevStep","playButton","nextStep","stepEditPanel","stepNameWrap","stepNamePreset","stepNameInput","newStepNaming","saveStep","addStep","deleteStep","actionMenuToggle","actionMenuClose","editPanel","currentStepTitle","court","validationLayer","movementLayer","ballPathLayer","playerLayer","ballObject","tapNotice","status","infoDialog","closeInfo","closeInfoBottom","infoSituation","lineupEditor","lineupGrid","liberoToggle","syncBadge","jsBuildBadge","passwordDialog","closePassword","cancelPassword","confirmPassword","passwordInput","passwordStatus","migrationPanel","dataSourceStatus","migrateLocalButton","migrationHint","viewToggle","view2d","view25d","positionInfoToggle","court25d","court25Floor","movement25Layer","ballPath25Layer","action25Layer","player25Layer","ball25Object","actionPanel","actionType","actionActorWrap","actionActor","actionTechniqueWrap","actionTechnique","actionHelperWrap","actionHelper","actionOutcomeWrap","actionOutcome","actionReasonWrap","actionReason","attackBlockWrap","attackBlocker1","attackBlocker2","snapBallToActor","clearAction","actionHint","actionSummary","actionLayer","ruleCheck","ruleCheckToggle","ruleCheckSummary","ruleCheckChevron","ruleCheckDetails"];
...
const freshTeam=(name="Hauptaufstellung")=>({name,teamConfig:{roles:{...defaultRoles},libero:false},rotations:Array.from({length:6},(_,r)=>({name:defaultSituationName(r),rotationOffset:r,steps:[initialStep(r)]}))});
...
function normalizeTeam(team,index){
  ...
  team.rotations.forEach((rotation,r)=>{if(!rotation.name){rotation.name=defaultSituationName(r);changed=true}const offset=Number.isFinite(Number(rotation.rotationOffset))?Number(rotation.rotationOffset)%6:r%6;if(rotation.rotationOffset!==offset){rotation.rotationOffset=offset;changed=true}if(!Array.isArray(rotation.steps)||!rotation.steps.length){rotation.steps=[initialStep(offset)];changed=true}rotation.steps.forEach(step=>{if(normalizeStep(step,rotation))changed=true})});
  return changed;
}
function migrate(){
  let changed=false;
...
let editing=false,selected=null,dragging=null,playing=false,animations=[],editorPassword=null,lineupOpen=false,teamConfigOpen=false,committedState=structuredClone(state),dirty=false,namingNewStep=false,actionMenuOpen=false;
...
const td=()=>state.teams[state.teamIndex],rd=()=>td().rotations[state.rotation],sd=()=>rd().steps[state.step],rname=n=>td().rotations[n]?.name||defaultSituationName(n),prot=p=>rot(p.base,rd().rotationOffset||0),pat=n=>ownRoster.find(p=>prot(p)===n);
...
function ruleIssues(){
  ...
  steps.forEach((step,i)=>{
    ...
    if(a.type==="serve"&&actor&&actorPos){
      ...
      const faults=receivingRotationFaults(step);if(faults.size){const receiver=actor.team==="own"?"Gegner":"eigene Mannschaft";add("error",i,`${step.name}: ${receiver} steht beim Aufschlag nicht rotationsgerecht.`)}
    }
...
function receivingRotationFaults(step){
  const a=step?.action||{},actor=playerById(a.actorId);if(a.type!=="serve"||!actor)return new Set();
  const receiverTeam=actor.team==="own"?"opponent":"own",bad=new Set();
  const playerAt=n=>receiverTeam==="own"?pat(n):opponentRoster.find(p=>p.base===n),pos=n=>step.positions[playerAt(n)?.id];
  const horizontal=[[4,3],[3,2],[5,6],[6,1]],vertical=[[4,5],[3,6],[2,1]];
  horizontal.forEach(([left,right])=>{const L=pos(left),R=pos(right),lp=playerAt(left),rp=playerAt(right);if(!L||!R||!lp||!rp)return;const invalid=receiverTeam==="own"?L.x>=R.x:R.x>=L.x;if(invalid){bad.add(lp.id);bad.add(rp.id)}});
  vertical.forEach(([front,back])=>{const F=pos(front),B=pos(back),fp=playerAt(front),bp=playerAt(back);if(!F||!B||!fp||!bp)return;const invalid=receiverTeam==="own"?F.y>=B.y:F.y<=B.y;if(invalid){bad.add(fp.id);bad.add(bp.id)}});
  return bad;
}
function validate(){
  e.validationLayer.innerHTML="";const bad=receivingRotationFaults(sd()),a=actionData(),actor=playerById(a.actorId);if(a.type!=="serve"||!actor||!bad.size)return bad;
  ...
  [[4,3],[3,2],[5,6],[6,1]].forEach(([l,r])=>{const L=pos(l),R=pos(r);if(!L||!R)return;if(bad.has(playerAt(l).id)||bad.has(playerAt(r).id)){const x=(L.x+R.x)/2;e.validationLayer.appendChild(svg("line",{x1:x,y1:receiverTeam==="own"?455:60,x2:x,y2:receiverTeam==="own"?840:445,class:"validation-invalid"}))}});
  [[4,5],[3,6],[2,1]].forEach(([f,b])=>{const F=pos(f),B=pos(b);if(!F||!B)return;if(bad.has(playerAt(f).id)||bad.has(playerAt(b).id)){const y=(F.y+B.y)/2;e.validationLayer.appendChild(svg("line",{x1:105,y1:y,x2:595,y2:y,class:"validation-invalid"}))}});
  return bad;
}
...
function renderTeamOptions(){
  if(!e.teamSelect)return;const current=String(state.teamIndex);e.teamSelect.innerHTML="";state.teams.forEach((team,i)=>{const o=document.createElement("option");o.value=String(i);o.textContent=team.name||`Teamaufstellung ${i+1}`;e.teamSelect.appendChild(o)});e.teamSelect.value=current;
}
function renderSituationOptions(){
  const current=String(state.rotation);e.rotationSelect.innerHTML="";td().rotations.forEach((situation,i)=>{const o=document.createElement("option");o.value=String(i);o.textContent=situation.name||defaultSituationName(i);e.rotationSelect.appendChild(o)});e.rotationSelect.value=current;e.situationNameInput.value=rd().name||defaultSituationName(state.rotation)
}
function render(){
  renderTeamOptions();renderSituationOptions();e.stepNumber.textContent=state.step+1;e.stepTotal.textContent=rd().steps.length;e.stepNameInput.value=sd().name;e.stepNamePreset.value=["Aufschlag","Annahme","Zuspiel","Angriff","Angriffssicherung","Block","Doppelblock","Abwehr"].includes(sd().name)?sd().name:"";e.currentStepTitle.textContent=sd().name;e.liberoToggle.checked=td().teamConfig.libero;document.body.classList.toggle("editing-mode",editing);updateMigrationUI();e.addTeam?.classList.toggle("hidden",!editing);e.deleteTeam?.classList.toggle("hidden",!editing);e.teamConfigToggle?.classList.toggle("hidden",!editing);e.addSituation.classList.toggle("hidden",!editing);e.deleteSituation.classList.toggle("hidden",!editing);e.addStep.classList.toggle("hidden",!editing);e.deleteStep.classList.toggle("hidden",!editing);e.actionMenuToggle.classList.toggle("hidden",!editing);e.saveStep.classList.toggle("hidden",!editing||!dirty);e.newStepNaming.classList.toggle("hidden",!editing||!namingNewStep);e.teamConfigPanel?.classList.toggle("hidden",!editing||!teamConfigOpen);e.lineupEditor?.classList.toggle("hidden",!editing||!teamConfigOpen);renderActionEditor();paths();const bad=validate();
  allPlayers.forEach(p=>{const g=e.playerLayer.querySelector(`[data-id="${p.id}"]`),pos=sd().positions[p.id],role=effectiveRole(p);g.setAttribute("transform",`translate(${pos.x} ${pos.y})`);g.classList.toggle("editable",editing);g.classList.toggle("selected",selected?.type==="player"&&selected.id===p.id);g.classList.toggle("position-warning",bad.has(p.id));g.querySelector("[data-role]").textContent=role;g.querySelector("circle").setAttribute("fill",p.team==="opponent"?"#e32828":role==="L"?"#111827":"#0b4fc6");const label=g.querySelector("[data-label]");label.textContent=p.team==="opponent"?`Gegner · P${p.base}`:role==="L"?`Libero · Position ${prot(p)}`:`${roleNames[role]||role} · Position ${prot(p)}`;label.style.display=showPositionInfo?"":"none"});
...
function edit(v){stop();clearVisualAnimations();if(v&&!editing){state=structuredClone(committedState);dirty=false}if(!v&&editing&&dirty){state=structuredClone(committedState);migrate();dirty=false}editing=v;selected=dragging=null;if(!v){namingNewStep=false;actionMenuOpen=false;teamConfigOpen=false;}e.editButton.textContent=v?"✓ Fertig":"✎ Bearbeiten";[e.stepEditPanel,e.editPanel].forEach(x=>x.classList.toggle("hidden",!v));e.tapNotice.classList.add("hidden");if(!v){lineupOpen=false;teamConfigOpen=false;e.lineupEditor.classList.add("hidden");e.teamConfigPanel?.classList.add("hidden")}render()}
...
e.teamSelect?.addEventListener("change",ev=>{state.teamIndex=Number(ev.target.value);state.rotation=0;state.step=0;namingNewStep=false;actionMenuOpen=false;teamConfigOpen=false;buildLineupEditor();render()});
e.addTeam?.addEventListener("click",()=>{if(!editing)return;const suggested=`Teamaufstellung ${state.teams.length+1}`;const entered=window.prompt("Name der neuen Teamaufstellung:",suggested);if(entered===null)return;const name=entered.trim()||suggested;const copyCurrent=window.confirm("Aktuelle Teamaufstellung als Vorlage kopieren?\n\nOK = aktuelle Aufstellung und Spielsituationen kopieren\nAbbrechen = neue Grundaufstellung anlegen");const team=copyCurrent?structuredClone(td()):freshTeam(name);team.name=name;state.teams.splice(state.teamIndex+1,0,team);state.teamIndex++;state.rotation=0;state.step=0;namingNewStep=false;actionMenuOpen=false;teamConfigOpen=false;dirty=true;buildLineupEditor();render()});
...
e.teamConfigToggle?.addEventListener("click",()=>{if(!editing)return;teamConfigOpen=!teamConfigOpen;e.teamConfigPanel?.classList.toggle("hidden",!teamConfigOpen);e.lineupEditor?.classList.toggle("hidden",!teamConfigOpen);if(teamConfigOpen)buildLineupEditor();});
e.teamConfigClose?.addEventListener("click",()=>{teamConfigOpen=false;e.teamConfigPanel?.classList.add("hidden")});
e.editButton.addEventListener("click",requestEdit);e.rotationSelect.addEventListener("change",x=>{state.rotation=Number(x.target.value);state.step=0;namingNewStep=false;actionMenuOpen=false;buildLineupEditor();render()});e.situationNameInput.addEventListener("input",()=>{});e.stepNamePreset.addEventListener("change",()=>{if(e.stepNamePreset.value){e.stepNameInput.value=e.stepNamePreset.value;dirty=true}});e.stepNameInput.addEventListener("input",()=>{e.stepNamePreset.value=["Aufschlag","Annahme","Zuspiel","Angriff","Angriffssicherung","Block","Doppelblock","Abwehr"].includes(e.stepNameInput.value.trim())?e.stepNameInput.value.trim():"";dirty=true});e.saveSituation.addEventListener("click",()=>{});e.addSituation.addEventListener("click",()=>{if(!editing)return;const suggested=`Neue Spielsituation ${td().rotations.length+1}`;const entered=window.prompt("Name der neuen Spielsituation:",suggested);if(entered===null)return;const name=entered.trim()||suggested;const copy=structuredClone(rd());copy.name=name;td().rotations.splice(state.rotation+1,0,copy);state.rotation++;state.step=0;namingNewStep=false;actionMenuOpen=false;dirty=true;buildLineupEditor();render()});e.deleteSituation.addEventListener("click",()=>{if(!editing)return;if(td().rotations.length===1){e.status.textContent="Die einzige Spielsituation kann nicht gelöscht werden.";return}const name=rd().name||defaultSituationName(state.rotation);if(!window.confirm(`Spielsituation „${name}“ wirklich löschen?\n\nDabei werden alle Schritte dieser Spielsituation gelöscht.`))return;td().rotations.splice(state.rotation,1);state.rotation=Math.max(0,state.rotation-1);state.step=0;namingNewStep=false;actionMenuOpen=false;dirty=true;buildLineupEditor();render()});
...
e.deleteStep.addEventListener("click",()=>{if(rd().steps.length===1){e.status.textContent="Der einzige Schritt kann nicht gelöscht werden.";return}rd().steps.splice(state.step,1);state.step=Math.max(0,state.step-1);namingNewStep=false;actionMenuOpen=false;dirty=true;render()});
...
async function checkForUpdate(){try{const res=await fetch(`version.json?_=${Date.now()}`,{cache:"no-store"});if(!res.ok)return;const remote=await res.json();if(remote?.version&&compareVersions(remote.version,VERSION)>0){const u=new URL(location.href);u.searchParams.set("v",remote.version);location.replace(u.toString())}}catch{}}
checkForUpdate();createPlayers();create25Floor();create25Players();buildLineupEditor();edit(false);loadRemote();
})();
