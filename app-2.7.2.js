(() => {
"use strict";
const VERSION="2.7.2",DATA_SCHEMA=5,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function renderStepStrip(){
  ...
  rd().steps.forEach((step,i)=>{
    const b=document.createElement("button");
    b.type="button";
    b.dataset.stepIndex=String(i);
    b.className=i===state.step?"active":"";
    const type=step?.action?.type||"";
    if(type)b.classList.add("has-action");
    if(type==="point")b.classList.add("point-step");
    b.textContent=`${i+1} · ${step.name||`Schritt ${i+1}`}`;
    b.title=`Schritt ${i+1}: ${step.name||`Schritt ${i+1}`}`;
