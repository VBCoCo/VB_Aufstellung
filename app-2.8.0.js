(() => {
"use strict";
const VERSION="2.8.0",DATA_SCHEMA=5,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function updateMigrationUI(){
  ...
  else if(offlineMeta.pendingLocalChanges)sourceText="Lokal – Änderungen warten auf Abgleich";
...
async function save(msg){
  committedState=structuredClone(state);dirty=false;localStorage.setItem(KEY,JSON.stringify(committedState));offlineMeta.lastLocalSaveAt=isoNow();offlineMeta.pendingLocalChanges=true;persistOfflineMeta();e.status.textContent=msg;
  if(navigator.onLine&&supabaseConfigured()&&editorPassword!==null){
    try{await rpc("save_trainer_state",{p_password:editorPassword,p_payload:state});remoteHasData=true;dataSource="supabase";offlineMeta.lastSuccessfulSyncAt=isoNow();offlineMeta.lastOnlineLoadAt=offlineMeta.lastSuccessfulSyncAt;offlineMeta.pendingLocalChanges=false;persistOfflineMeta();e.syncBadge.textContent="Speicherung: Supabase synchron"}
    catch(err){dataSource="browser";e.syncBadge.textContent="Lokal gespeichert; Supabase nicht erreichbar";e.status.textContent=`${msg} Lokal gespeichert. Supabase: ${err.message}`}
  }else if(!navigator.onLine){dataSource="browser";e.syncBadge.textContent="Offline – lokal gespeichert";e.status.textContent=`${msg} Offline lokal gespeichert.`}
  else if(offlineMeta.pendingLocalChanges){dataSource="browser";e.syncBadge.textContent="Lokal gespeichert – noch nicht synchronisiert"}
  updateMigrationUI();
}
...
function receivingRotationFaults(step,rotation=rd()){
...
function renderSituationOptions(){
  ...
  td().rotations.forEach((situation,i)=>{
    ...
    o.value=String(i);o.textContent=situation.name||defaultSituationName(i);o.dataset.status=status;
  ...
  e.rotationSelect.value=current;e.situationNameInput.value=rd().name||defaultSituationName(state.rotation);
...
function edit(v){stop();clearVisualAnimations();if(v&&!editing){state=structuredClone(committedState);dirty=false}if(!v&&editing&&dirty){state=structuredClone(committedState);migrate();dirty=false}editing=v;selected=dragging=null;if(!v){namingNewStep=false;actionMenuOpen=false;teamConfigOpen=false;}e.editButton.textContent=v?"✓":"✎";e.editButton.setAttribute("aria-label",v?"Bearbeitung beenden":"Bearbeiten");e.editButton.setAttribute("title",v?"Bearbeitung beenden":"Bearbeiten");e.editPanel.classList.toggle("hidden",!v);e.stepEditPanel.classList.toggle("hidden",!v||!namingNewStep);e.tapNotice.classList.add("hidden");if(!v){lineupOpen=false;teamConfigOpen=false;e.lineupEditor.classList.add("hidden");e.teamConfigPanel?.classList.add("hidden")}render()}
...
window.addEventListener("offline",()=>{dataSource="browser";e.syncBadge.textContent="Offline – lokale Daten";updateMigrationUI()});
window.addEventListener("online",()=>{e.syncBadge.textContent=offlineMeta.pendingLocalChanges?"Online – lokale Änderungen warten auf Abgleich":"Online – Supabase wird geprüft";updateMigrationUI();if(!offlineMeta.pendingLocalChanges)loadRemote()});
registerOfflineWorker();checkForUpdate();createPlayers();create25Floor();create25Players();buildLineupEditor();edit(false);loadRemote();
})();
