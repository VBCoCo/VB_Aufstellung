(() => {
"use strict";
const VERSION="2.9.0",DATA_SCHEMA=6,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
async function save(msg){
  if(!navigator.onLine||!supabaseConfigured()||editorPassword===null){e.status.textContent="Speichern nicht möglich: Zum Bearbeiten und Speichern wird eine Supabase-Verbindung benötigt.";updateMigrationUI();render();return}
  try{await rpc("save_trainer_state",{p_password:editorPassword,p_payload:state});remoteHasData=true;dataSource="supabase";committedState=structuredClone(state);dirty=false;localStorage.setItem(KEY,JSON.stringify(committedState));offlineMeta.lastSuccessfulSyncAt=isoNow();offlineMeta.lastOnlineLoadAt=offlineMeta.lastSuccessfulSyncAt;offlineMeta.lastLocalSaveAt=offlineMeta.lastSuccessfulSyncAt;offlineMeta.pendingLocalChanges=false;persistOfflineMeta();e.syncBadge.textContent="Speicherung: Supabase synchron";e.status.textContent=msg}
...
function validate(){
  e.validationLayer.innerHTML="";if(tacticMode)return new Set();const bad=receivingRotationFaults(sd()),a=actionData(),actor=playerById(a.actorId);if(a.type!=="serve"||!actor||!bad.size)return bad;
...
function renderSituationOptions(){
  ...
  e.rotationSelect.value=current;e.situationNameInput.value=situationDisplayName(rd());
...
function render(){
  ...
  if(tacticMode&&e.tacticTitle)e.tacticTitle.textContent=`Taktiktafel · ${td().name} · ${rname(state.rotation)} · Schritt ${state.step+1}: ${sd().name} · Änderungen werden nicht gespeichert`;
  if(tacticMode){if(e.tacticPrev)e.tacticPrev.disabled=state.step<=0;if(e.tacticNext)e.tacticNext.disabled=state.step>=rd().steps.length-1;}
...
async function requestEdit(){if(editing){edit(false);return}if(!navigator.onLine){e.status.textContent="Offline-Bearbeitung nicht möglich. Zum Bearbeiten wird eine Verbindung zu Supabase benötigt.";window.alert("Offline-Bearbeitung nicht möglich.\n\nZum Bearbeiten und Speichern wird eine Internetverbindung zu Supabase benötigt.");return}if(!supabaseConfigured()){e.status.textContent="Bearbeiten nicht möglich: Supabase ist nicht konfiguriert.";return}e.passwordInput.value="";e.passwordStatus.textContent="";e.passwordDialog.showModal();setTimeout(()=>e.passwordInput.focus(),50)}
...
function enterTacticMode(){
  ...
  tacticPreviousView=preferredView;tacticMode=true;selected=dragging=null;preferredView="2d";
  e.status.textContent="Taktiktafel: Änderungen werden nicht gespeichert.";
  render();
}
function restoreTacticStep(index=state.step){
  ...
  state.step=next;selected=dragging=null;
  render();
}
function exitTacticMode(){
  ...
  tacticSourceState=null;tacticMode=false;selected=dragging=null;preferredView=tacticPreviousView;
...
e.court.addEventListener("pointermove",ev=>{if(!dragging||(!editing&&!tacticMode)||playing)return;ev.preventDefault();const p=point(ev);if(tacticMode){if(dragging.type==="player")sd().positions[dragging.id]={x:clamp(p.x,32,668),y:clamp(p.y,32,868)};else sd().ball=cb(p);render();return;}if(dragging.type==="player"){const player=allPlayers.find(x=>x.id===dragging.id);sd().positions[dragging.id]=playerPoint(p,player);if(actionData().actorId===dragging.id)syncContactBall();dirty=true}else{sd().ball=cb(p);dirty=true}render()},{passive:false});["pointerup","pointercancel"].forEach(n=>e.court.addEventListener(n,()=>dragging=null));
...
e.teamNameInput?.addEventListener("input",()=>{if(!editing)return;td().name=e.teamNameInput.value.trimStart()||"Teamaufstellung";dirty=true;renderTeamOptions();e.saveStep.classList.remove("hidden")});
