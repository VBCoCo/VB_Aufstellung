(() => {
"use strict";
const VERSION="3.0.0",DATA_SCHEMA=6,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function renderAccount(){if(!e.accountRoles)return;const t=selectedTeam();e.accountName.textContent=userAccess?.display_name||authSession?.user?.email||"Konto";e.accountEmail.textContent=authSession?.user?.email||"";const roles=[];if(userAccess?.platform_admin)roles.push(`Superadmin${userAccess.platform_admin_code?` · ${userAccess.platform_admin_code}`:''}`);(t?.roles||[]).forEach(r=>roles.push(r==='club_admin'?'Vereinsadmin':r==='editor'?'Bearbeiter':'Viewer'));e.accountRoles.innerHTML=roles.map(r=>`<span class="role-chip">${r}</span>`).join('');const teams=allTeams();e.clubSwitcherWrap.classList.toggle('hidden',teams.length<2);e.teamContextSelect.innerHTML=teams.map(x=>`<option value="${x.id}" ${x.id===t?.id?'selected':''}>${x.club_name} · ${x.name}</option>`).join('')}
...
function showApp(){document.body.classList.remove('auth-pending');e.authGate.classList.add('hidden');e.appMain.classList.remove('auth-hidden');e.editButton.classList.toggle('hidden',!canEdit());renderAccount()}
function showLogin(msg=''){e.authGate.classList.remove('hidden');e.appMain.classList.add('auth-hidden');e.authStatus.textContent=msg}
...
e.accountButton?.addEventListener('click',()=>{renderAccount();e.accountDialog.showModal()});e.closeAccount?.addEventListener('click',()=>e.accountDialog.close());e.logoutButton?.addEventListener('click',async()=>{persistSession(null);userAccess=null;localStorage.removeItem(ACCESS_KEY);e.accountDialog.close();showLogin('Du wurdest abgemeldet.')});e.teamContextSelect?.addEventListener('change',async ev=>{currentTeamId=ev.target.value;localStorage.setItem(TEAM_KEY,currentTeamId);e.accountDialog.close();edit(false);await loadRemote();renderAccount()});
bootV3();
})();
