(() => {
"use strict";
const VERSION="3.0.3",DATA_SCHEMA=6,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function roleEditor(m,mode){const roles=m.roles||[],isSelf=m.user_id===authSession?.user?.id;const allowClubAdmin=mode==='platform';const checks=[['viewer','Viewer'],['editor','Bearbeiter'],...(allowClubAdmin?[['club_admin','Vereinsadmin']]:[])].map(([value,label])=>`<label class="admin-role-check"><input type="checkbox" value="${value}" ${roles.includes(value)?'checked':''}>${label}</label>`).join('');const reset=`<button class="admin-password-reset" type="button" data-membership-id="${esc(m.membership_id)}" data-email="${esc(m.email||'')}">Passwort-Mail</button>`;const remove=mode==='platform'?`<button class="admin-delete-user" type="button" data-user-id="${esc(m.user_id)}" data-email="${esc(m.email||'')}" ${isSelf?'disabled title="Das eigene Konto kann hier nicht gelöscht werden."':''}>Endgültig löschen</button>`:'';return `<div class="admin-member-actions"><div class="admin-role-editor" data-membership-id="${esc(m.membership_id)}">${checks}<button class="admin-save-roles" type="button" ${isSelf?'disabled title="Eigene Rollen werden aus Sicherheitsgründen hier nicht geändert."':''}>Rollen speichern</button></div><div class="admin-member-buttons">${reset}<button class="admin-toggle-member ${m.active===false?'reactivate':''}" type="button" data-membership-id="${esc(m.membership_id)}" data-active="${m.active===false?'true':'false'}" ${isSelf?'disabled title="Der eigene Vereinszugang kann hier nicht deaktiviert werden."':''}>${m.active===false?'Reaktivieren':'Deaktivieren'}</button>${remove}</div></div>`}
function memberRow(m,mode='view'){const name=m.display_name||m.email||'Benutzer';return `<div class="admin-member-wrap"><div class="admin-member"><div class="admin-member-main"><strong>${esc(name)}</strong><span>${esc(m.email||'')}</span></div><div class="admin-member-roles">${renderRoleChips(m.roles||[])}</div><span class="admin-state ${m.active===false?'inactive':''}">${m.active===false?'inaktiv':'aktiv'}</span></div>${mode==='view'?'':roleEditor(m,mode)}</div>`}
function inviteForm(clubId,mode){const roleOptions=mode==='platform'?`<label><input type="checkbox" name="roles" value="club_admin" checked> Vereinsadmin</label><label><input type="checkbox" name="roles" value="viewer"> Viewer</label><label><input type="checkbox" name="roles" value="editor"> Bearbeiter</label>`:`<label><input type="checkbox" name="roles" value="viewer" checked> Viewer</label><label><input type="checkbox" name="roles" value="editor"> Bearbeiter</label>`;return `<form class="admin-invite-form" data-club-id="${esc(clubId)}"><div class="admin-invite-title">Benutzer einladen</div><input type="email" name="email" required autocomplete="email" placeholder="E-Mail-Adresse"><div class="admin-invite-roles">${roleOptions}</div><button class="primary" type="submit">Einladung senden</button><p class="admin-action-status" aria-live="polite"></p></form>`}
function renderPlatformOverview(data){const clubs=data?.clubs||[],admins=data?.platform_admins||[];const memberCount=clubs.reduce((n,c)=>n+(c.members?.length||0),0);e.platformAdminContent.innerHTML=`<div class="admin-summary"><div><strong>${clubs.length}</strong><span>Vereine</span></div><div><strong>${memberCount}</strong><span>Mitgliedschaften</span></div><div><strong>${admins.length}</strong><span>Superadmins</span></div></div><section class="admin-section"><h3>Superadmins</h3>${admins.map(a=>`<div class="admin-member"><div class="admin-member-main"><strong>${esc(a.admin_code)}</strong><span>${esc(a.display_name||a.email||'')}</span><small>${esc(a.email||'')}</small></div>${a.active===false?'<span class="admin-state inactive">inaktiv</span>':'<span class="admin-state">aktiv</span>'}</div>`).join('')||'<p class="hint">Keine Superadmins gefunden.</p>'}</section><section class="admin-section"><h3>Vereine</h3>${clubs.map(c=>`<article class="admin-club"><div class="admin-club-head"><div><strong>${esc(c.name)}</strong><span>${esc(c.slug)}</span></div><span class="admin-state ${c.active===false?'inactive':''}">${c.active===false?'inaktiv':'aktiv'}</span></div><div class="admin-club-meta">${Number(c.team_count)||0} Mannschaft(en) · ${c.members?.length||0} Benutzer</div>${inviteForm(c.id,'platform')}<div class="admin-member-list">${(c.members||[]).map(m=>memberRow(m,'platform')).join('')||'<p class="hint">Noch keine Benutzer.</p>'}</div></article>`).join('')||'<p class="hint">Keine Vereine vorhanden.</p>'}</section>`;wireAdminActions(e.platformAdminContent,'platform')}
function renderClubOverview(data){const c=data?.club||{},teams=data?.teams||[],members=data?.members||[];e.clubAdminContent.innerHTML=`<div class="admin-club-title"><strong>${esc(c.name||'Verein')}</strong><span>${esc(c.slug||'')}</span></div><div class="admin-summary"><div><strong>${teams.length}</strong><span>Mannschaften</span></div><div><strong>${members.length}</strong><span>Benutzer</span></div></div><section class="admin-section"><h3>Mannschaften</h3><div class="admin-team-list">${teams.map(t=>`<div class="admin-team"><strong>${esc(t.name)}</strong><span class="admin-state ${t.active===false?'inactive':''}">${t.active===false?'inaktiv':'aktiv'}</span></div>`).join('')||'<p class="hint">Keine Mannschaften vorhanden.</p>'}</div></section><section class="admin-section"><h3>Benutzer & Rollen</h3>${inviteForm(c.id,'club')}<div class="admin-member-list">${members.map(m=>memberRow(m,'club')).join('')||'<p class="hint">Noch keine Benutzer.</p>'}</div></section>`;wireAdminActions(e.clubAdminContent,'club')}
...
function checkedRoles(container){return [...container.querySelectorAll('input[type="checkbox"]:checked')].map(x=>x.value)}
function wireAdminActions(container,mode){container.querySelectorAll('.admin-invite-form').forEach(form=>form.addEventListener('submit',async ev=>{ev.preventDefault();const status=form.querySelector('.admin-action-status'),button=form.querySelector('button[type="submit"]');const email=form.elements.email.value.trim(),roles=checkedRoles(form.querySelector('.admin-invite-roles'));if(!roles.length){status.textContent='Bitte mindestens eine Rolle wählen.';return}button.disabled=true;status.textContent='Einladung wird gesendet …';try{const result=await edgeAdminAction('invite_member',{club_id:form.dataset.clubId,email,roles});status.textContent=result.invited?'Einladung wurde gesendet.':'Benutzer existiert bereits und wurde dem Verein zugeordnet.';form.reset();if(mode==='platform')await refreshPlatformAdmin();else await refreshClubAdmin()}catch(err){status.textContent=err.message}finally{button.disabled=false}}));container.querySelectorAll('.admin-save-roles').forEach(button=>button.addEventListener('click',async()=>{const editor=button.closest('.admin-role-editor'),roles=checkedRoles(editor);if(!roles.length){alert('Mindestens eine Rolle muss aktiv bleiben.');return}button.disabled=true;try{await edgeAdminAction('set_roles',{membership_id:editor.dataset.membershipId,roles});if(mode==='platform')await refreshPlatformAdmin();else await refreshClubAdmin()}catch(err){alert(err.message);button.disabled=false}}));container.querySelectorAll('.admin-toggle-member').forEach(button=>button.addEventListener('click',async()=>{const activate=button.dataset.active==='true';if(!confirm(activate?'Diesen Vereinszugang wieder aktivieren?':'Diesen Vereinszugang wirklich deaktivieren?'))return;button.disabled=true;try{await edgeAdminAction('set_membership_active',{membership_id:button.dataset.membershipId,active:activate});if(mode==='platform')await refreshPlatformAdmin();else await refreshClubAdmin()}catch(err){alert(err.message);button.disabled=false}}));container.querySelectorAll('.admin-password-reset').forEach(button=>button.addEventListener('click',async()=>{const email=button.dataset.email||'dieses Konto';if(!confirm(`Passwort-Mail an ${email} senden?`))return;button.disabled=true;try{await edgeAdminAction('send_password_reset',{membership_id:button.dataset.membershipId});alert(`Passwort-Mail wurde an ${email} angefordert.\n\nBei häufigen Tests kann das Supabase-Mail-Limit greifen.`)}catch(err){alert(err.message)}finally{button.disabled=false}}));container.querySelectorAll('.admin-delete-user').forEach(button=>button.addEventListener('click',async()=>{const email=button.dataset.email||'';if(!confirm(`Benutzerkonto ${email||''} wirklich ENDGÜLTIG löschen?\n\nDer Benutzer verliert alle Vereinszuordnungen. Diese Aktion kann nicht rückgängig gemacht werden.`))return;const typed=prompt(`Zur Sicherheit bitte die E-Mail-Adresse exakt eingeben:\n${email}`,'');if(typed!==email){if(typed!==null)alert('E-Mail-Adresse stimmt nicht überein. Benutzer wurde nicht gelöscht.');return}button.disabled=true;try{await edgeAdminAction('delete_user',{user_id:button.dataset.userId});alert(`Benutzer ${email} wurde endgültig gelöscht.`);await refreshPlatformAdmin()}catch(err){alert(err.message);button.disabled=false}}))}
