(() => {
"use strict";
const VERSION="3.0.4.10",DATA_SCHEMA=8,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
e.saveStep.addEventListener("click",()=>{if(namingNewStep)sd().name=e.stepNameInput.value.trim()||`Schritt ${state.step+1}`;keepServePlayersInside();selected=dragging=null;e.tapNotice.classList.add("hidden");save("Änderungen gespeichert.");namingNewStep=false;render()});
...
function compactContextQuestion(q){
  const msgs=q.messages||[],first=msgs[0],preview=(first?.body||'Frage').trim(),viewerMode=!editing,unread=Math.max(0,Number(q.unread_messages)||0);
  const asker=viewerMode?'Frage':(q.author_name||'Viewer');
  const messageHtml=msgs.map((m,i)=>{const who=viewerMode?(m.author_is_editor?'Bearbeiter':(i===0?'Deine Frage':'Du')):(m.author_name||(m.author_is_editor?'Bearbeiter':'Viewer'));return `<div class="question-message ${m.author_is_editor?'editor':'viewer'}"><strong>${esc(who)}</strong><p>${esc(m.body)}</p><small>${new Date(m.created_at).toLocaleString('de-DE')}</small>${editing&&canEdit()?`<button class="question-as-info" data-message-id="${esc(m.id)}" data-question-id="${esc(q.id)}" type="button">ⓘ Als Info übernehmen</button>`:''}</div>`}).join('');
  return `<details class="question-context-item ${unread?'has-unread':''}" data-question-id="${esc(q.id)}"><summary><div><strong>${esc(asker)} · ${questionStatusLabel(q.status)}</strong><span>${esc(preview.length>90?preview.slice(0,90)+' …':preview)}</span></div>${unread?`<b>${unread} neu</b>`:''}</summary><div class="question-context-thread">${messageHtml}<textarea class="question-reply" rows="2" maxlength="2000" placeholder="${canEdit()?'Antwort schreiben …':'Rückfrage schreiben …'}"></textarea><div class="question-actions"><button class="question-reply-send primary" type="button">Senden</button>${canEdit()?'<button class="question-resolve ghost" type="button">Erledigt</button>':''}${questionDeleteButton(q)}</div></div></details>`;
}
function renderQuestionContext(){
  ...
  e.questionContextPanel.innerHTML=`<div class="question-context-head"><div><span class="eyebrow">Fragen zu Schritt ${state.step+1}</span><strong>${qs.length?`${qs.length} aktive Frage${qs.length===1?'':'n'}`:'Noch keine Frage'}</strong></div><button class="question-context-close" type="button" aria-label="Fragen ausblenden">✕</button></div><div class="question-context-items">${qs.map(compactContextQuestion).join('')||'<p class="hint">Zu diesem Schritt gibt es noch keine aktive Frage.</p>'}</div><div class="question-context-footer"><button class="question-context-new ghost" type="button">＋ Frage stellen</button></div>`;
