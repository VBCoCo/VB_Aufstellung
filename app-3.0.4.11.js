(() => {
"use strict";
const VERSION="3.0.4.11",DATA_SCHEMA=8,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function compactContextQuestion(q){
  const msgs=q.messages||[],first=msgs[0],preview=(first?.body||'Frage').trim(),viewerMode=!editing,unread=Math.max(0,Number(q.unread_messages)||0),manage=editing&&canEdit();
  const asker=viewerMode?'':(q.author_name||'Viewer');
  const messageHtml=msgs.map((m,i)=>{const who=viewerMode?(m.author_is_editor?'Bearbeiter':(i===0?'':'Du')):(m.author_name||(m.author_is_editor?'Bearbeiter':'Viewer'));return `<div class="question-message ${m.author_is_editor?'editor':'viewer'}">${who?`<strong>${esc(who)}</strong>`:''}<p>${esc(m.body)}</p>${viewerMode?'':`<small>${new Date(m.created_at).toLocaleString('de-DE')}</small>`}${manage?`<button class="question-as-info" data-message-id="${esc(m.id)}" data-question-id="${esc(q.id)}" type="button">ⓘ Als Info übernehmen</button>`:''}</div>`}).join('');
  return `<details class="question-context-item ${viewerMode?'viewer-compact':''} ${unread?'has-unread':''}" data-question-id="${esc(q.id)}"><summary><div>${asker?`<strong>${esc(asker)} · ${questionStatusLabel(q.status)}</strong>`:''}<span>${esc(preview.length>110?preview.slice(0,110)+' …':preview)}</span></div>${unread?`<b>${unread} neu</b>`:''}</summary><div class="question-context-thread">${messageHtml}<textarea class="question-reply" rows="2" maxlength="2000" placeholder="${manage?'Antwort schreiben …':'Rückfrage schreiben …'}"></textarea><div class="question-actions"><button class="question-reply-send primary" type="button">Senden</button>${manage?'<button class="question-resolve ghost" type="button">Erledigt</button>':''}${questionDeleteButton(q)}</div></div></details>`;
}
function renderQuestionContext(){
  ...
  e.questionContextPanel.innerHTML=`${head}<div class="question-context-items">${qs.map(compactContextQuestion).join('')||'<p class="hint">Zu diesem Schritt gibt es noch keine aktive Frage.</p>'}</div><div class="question-context-footer"><button class="question-context-new ghost" type="button">＋ Frage stellen</button></div>`;
  openIds.forEach(id=>{const item=e.questionContextPanel.querySelector(`[data-question-id="${CSS.escape(id)}"]`);if(item)item.open=true});
