(() => {
"use strict";
const VERSION="3.0.4.15",DATA_SCHEMA=8,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function populateStepNamePreset(){if(!e.stepNamePreset)return;const current=e.stepNamePreset.value;e.stepNamePreset.innerHTML=`<option value="">– frei benennen –</option><optgroup label="Standard">${STEP_NAME_STANDARDS.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join("")}</optgroup>${recentStepNames().length?`<optgroup label="Zuletzt verwendet">${recentStepNames().map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join("")}</optgroup>`:""}`;if([...e.stepNamePreset.options].some(o=>o.value===current))e.stepNamePreset.value=current}
...
function render25(){
  ...
  e.view2d.classList.toggle("active",preferredView==="2d");e.view25d.classList.toggle("active",preferredView==="25d");if(e.positionInfoToggle){e.positionInfoToggle.classList.toggle("active",showPositionInfo);e.positionInfoToggle.textContent=`Positionsnummern ${showPositionInfo?"an":"aus"}`;e.positionInfoToggle.setAttribute("aria-pressed",String(showPositionInfo))}
  if(!visible)return;
...
function goStep(target){questionContextOpen=false;activeQuestionId=null;target=Math.max(0,Math.min(rd().steps.length-1,target));if(target===state.step)return;if(editing){clearVisualAnimations();state.step=target;selected=dragging=null;namingNewStep=false;actionMenuOpen=false;render();return}animate(target)}
...
e.saveStep.addEventListener("click",()=>{if(namingNewStep){sd().name=e.stepNameInput.value.trim()||`Schritt ${state.step+1}`;rememberStepName(sd().name)};keepServePlayersInside();selected=dragging=null;e.tapNotice.classList.add("hidden");save("Änderungen gespeichert.");namingNewStep=false;render()});
e.addStep.addEventListener("click",()=>{keepServePlayersInside();const s=sd();rd().steps.splice(state.step+1,0,{name:`Schritt ${state.step+2}`,positions:structuredClone(s.positions),ball:structuredClone(s.ball),action:{type:"",actorId:"",technique:"",helperId:"",blocker1Id:"",blocker2Id:"",outcome:"",reason:"",ballLinked:false}});state.step++;namingNewStep=true;actionMenuOpen=false;dirty=true;render();setTimeout(()=>e.stepNamePreset.focus(),0)});
...
let questionCache=[],activeQuestionId=null,questionContextOpen=false,questionRefreshBusy=false;
...
function renderQuestionContext(){
  ...
  e.questionContextPanel.querySelector('.question-context-new')?.addEventListener('click',()=>{questionContextOpen=false;renderQuestionContext();openQuestions(true)});
