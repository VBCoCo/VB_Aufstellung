(() => {
"use strict";
const VERSION="3.0.4.16",DATA_SCHEMA=8,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
let editing=false,selected=null,dragging=null,playing=false,animations=[],editorPassword=null,lineupOpen=false,teamConfigOpen=false,situationConfigOpen=false,committedState=structuredClone(state),dirty=false,namingNewStep=false,renamingStep=false,actionMenuOpen=false,tacticMode=false,tacticSourceState=null,tacticPreviousView="2d",tacticChanged=false;
...
function edit(v){stop();clearVisualAnimations();if(v&&!editing){state=structuredClone(committedState);dirty=false}if(!v&&editing&&dirty){state=structuredClone(committedState);migrate();dirty=false}editing=v;selected=dragging=null;if(!v){namingNewStep=false;renamingStep=false;actionMenuOpen=false;teamConfigOpen=false;situationConfigOpen=false;}e.editButton.textContent=v?"✓":"✎";e.editButton.setAttribute("aria-label",v?"Bearbeitung beenden":"Bearbeiten");e.editButton.setAttribute("title",v?"Bearbeitung beenden":"Bearbeiten");e.editPanel.classList.toggle("hidden",!v);e.stepEditPanel.classList.add("hidden");e.tapNotice.classList.add("hidden");if(!v){lineupOpen=false;teamConfigOpen=false;situationConfigOpen=false;e.lineupEditor.classList.add("hidden");e.teamConfigPanel?.classList.add("hidden");e.situationConfigPanel?.classList.add("hidden")}render()}
...
function goStep(target){questionContextOpen=false;activeQuestionId=null;target=Math.max(0,Math.min(rd().steps.length-1,target));if(target===state.step)return;if(editing){clearVisualAnimations();state.step=target;selected=dragging=null;namingNewStep=false;renamingStep=false;actionMenuOpen=false;render();return}animate(target)}
e.stepStrip?.addEventListener("click",ev=>{questionContextOpen=false;activeQuestionId=null;const b=ev.target.closest("button[data-step-index]");if(!b||playing)return;const target=Number(b.dataset.stepIndex);if(!Number.isFinite(target)||target===state.step)return;clearVisualAnimations();state.step=target;selected=dragging=null;namingNewStep=false;renamingStep=false;actionMenuOpen=false;render()});
...
e.saveStep.addEventListener("click",()=>{if(renamingStep)applyInlineStepName(true);keepServePlayersInside();selected=dragging=null;e.tapNotice.classList.add("hidden");save("Änderungen gespeichert.");namingNewStep=false;renamingStep=false;render()});
e.addStep.addEventListener("click",()=>{keepServePlayersInside();const s=sd();rd().steps.splice(state.step+1,0,{name:`Schritt ${state.step+2}`,positions:structuredClone(s.positions),ball:structuredClone(s.ball),action:{type:"",actorId:"",technique:"",helperId:"",blocker1Id:"",blocker2Id:"",outcome:"",reason:"",ballLinked:false}});state.step++;namingNewStep=false;renamingStep=true;actionMenuOpen=false;dirty=true;render();setTimeout(()=>{if(e.stepNameInlineInput){e.stepNameInlineInput.focus();e.stepNameInlineInput.select()}},0)});
