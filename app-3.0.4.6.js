(() => {
"use strict";
const VERSION="3.0.4.6",DATA_SCHEMA=7,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
e.questionsButton?.addEventListener('click',openQuestions);e.closeQuestions?.addEventListener('click',()=>e.questionsDialog.close());e.questionNewButton?.addEventListener('click',()=>e.questionComposer.classList.toggle('hidden'));e.questionCancel?.addEventListener('click',()=>{e.questionComposer.classList.add('hidden');e.questionText.value=''});e.questionSend?.addEventListener('click',async()=>{const body=e.questionText.value.trim();if(!body)return;e.questionSend.disabled=true;try{await rpc('vt_create_question',{p_team_id:currentTeamId,p_situation_index:state.rotation,p_step_index:state.step,p_situation_name:rname(state.rotation),p_step_name:sd().name,p_body:body});e.questionText.value='';e.questionComposer.classList.add('hidden');await loadQuestions()}catch(err){alert(err.message)}finally{e.questionSend.disabled=false}});e.situationInfoButton?.addEventListener('click',()=>openContextInfo('situation'));e.stepInfoButton?.addEventListener('click',()=>openContextInfo('step'));e.closeContextInfo?.addEventListener('click',()=>e.contextInfoDialog.close());e.situationInfoEdit?.addEventListener('input',()=>{if(editing){rd().info=e.situationInfoEdit.value;dirty=true}});e.stepInfoEdit?.addEventListener('input',()=>{if(editing){sd().info=e.stepInfoEdit.value;dirty=true}});e.publishSituation?.addEventListener('click',async()=>{if(!editing)return;const issues=ruleIssues(rd());if(issues.some(x=>x.severity==='error')){alert('Freigabe nicht möglich: Diese Spielsituation enthält noch rote Regelfehler.');return}if(!rd().published&&(issues.some(x=>x.severity==='warning')||rd().steps.some((s,i)=>stepNeedsWork(s,i,rd())))&&!confirm('Diese Spielsituation enthält noch gelbe Hinweise oder unvollständige Schritte. Trotzdem freigeben?'))return;rd().published=!rd().published;dirty=true;await save(rd().published?'Spielsituation für Viewer freigegeben.':'Freigabe zurückgenommen.');render()});
...
let questionCache=[];
async function loadQuestions(){if(!navigator.onLine||!currentTeamId)return[];questionCache=await rpc("vt_list_questions",{p_team_id:currentTeamId})||[];renderQuestions();return questionCache}
...
function wireQuestionActions(){e.questionsContent?.querySelectorAll('.question-jump').forEach(b=>b.addEventListener('click',()=>{const target=Math.max(0,Math.min(td().rotations.length-1,Number(b.dataset.situation)));if(!editing&&!td().rotations[target]?.published){alert(canEdit()?'Diese Spielsituation ist aktuell nicht für Viewer freigegeben. Öffne sie im Bearbeiten-Modus.':'Diese Spielsituation ist aktuell nicht freigegeben.');return}state.rotation=target;state.step=Math.max(0,Math.min(rd().steps.length-1,Number(b.dataset.step)));e.questionsDialog.close();render();e.status.textContent='Frage-Kontext geöffnet.'}));e.questionsContent?.querySelectorAll('.question-reply-send').forEach(b=>b.addEventListener('click',async()=>{const card=b.closest('.question-card'),ta=card.querySelector('.question-reply'),body=ta.value.trim();if(!body)return;b.disabled=true;try{await rpc('vt_add_question_message',{p_question_id:card.dataset.questionId,p_body:body});await loadQuestions()}catch(err){alert(err.message)}finally{b.disabled=false}}));e.questionsContent?.querySelectorAll('.question-resolve').forEach(b=>b.addEventListener('click',async()=>{const id=b.closest('.question-card').dataset.questionId;await rpc('vt_set_question_status',{p_question_id:id,p_status:'resolved'});await loadQuestions()}));e.questionsContent?.querySelectorAll('.question-as-info').forEach(b=>b.addEventListener('click',()=>{const q=questionCache.find(x=>x.id===b.dataset.questionId),m=q?.messages?.find(x=>x.id===b.dataset.messageId);if(!q||!m)return;const target=confirm('OK = als Info zu diesem Schritt übernehmen.\nAbbrechen = als allgemeine Info zur Spielsituation übernehmen.')?'step':'situation';state.rotation=Number(q.situation_index);state.step=Number(q.step_index);if(target==='step')sd().info=m.body;else rd().info=m.body;dirty=true;e.questionsDialog.close();edit(true);render();e.status.textContent='Antwort als Info übernommen. Bitte speichern.'}))}
async function openQuestions(){e.questionsDialog.showModal();e.questionsContent.innerHTML='<p class="hint">Fragen werden geladen …</p>';try{await loadQuestions()}catch(err){e.questionsContent.innerHTML=`<p class="admin-error">${esc(err.message)}</p>`}}
