(() => {
"use strict";
const VERSION="3.0.4.7",DATA_SCHEMA=7,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function questionCard(q){const msgs=q.messages||[],last=msgs[msgs.length-1],preview=(last?.body||q.question||'').slice(0,90);return `<details class="question-card" data-question-id="${esc(q.id)}"><summary><div><strong>${esc(q.author_name||'Viewer')}</strong><span>${esc(q.situation_name)} · Schritt ${Number(q.step_index)+1}</span><small>${esc(preview)}${preview.length>=90?' …':''}</small></div><b>${questionStatusLabel(q.status)}</b></summary><div class="question-thread">${msgs.map(m=>`<div class="question-message ${m.author_is_editor?'editor':'viewer'}"><strong>${esc(m.author_name|| (m.author_is_editor?'Bearbeiter':'Viewer'))}</strong><p>${esc(m.body)}</p><small>${new Date(m.created_at).toLocaleString('de-DE')}</small>${canEdit()?`<button class="question-as-info" data-message-id="${esc(m.id)}" data-question-id="${esc(q.id)}" type="button">ⓘ Als Info übernehmen</button>`:''}</div>`).join('')}<div class="question-context"><span>${esc(q.situation_name)} · Schritt ${Number(q.step_index)+1} · ${esc(q.step_name||'')}</span><button class="question-jump" data-question-id="${esc(q.id)}" data-situation="${q.situation_index}" data-step="${q.step_index}" type="button">Zur Situation</button></div>${q.status!=='resolved'?`<textarea class="question-reply" rows="3" maxlength="2000" placeholder="${canEdit()?'Antwort schreiben …':'Rückfrage schreiben …'}"></textarea><div class="question-actions"><button class="question-reply-send primary" type="button">Senden</button>${canEdit()?'<button class="question-resolve ghost" type="button">Als erledigt markieren</button>':''}${questionDeleteButton(q)}</div>`:`<div class="question-actions"><p class="hint">Dieser Thread ist erledigt.</p>${questionDeleteButton(q)}</div>`}</div></details>`}
function renderQuestions(){if(!e.questionsContent)return;e.questionsContent.innerHTML=questionGroups();const open=questionCache.filter(q=>q.status==='open').length;if(e.questionsBadge){e.questionsBadge.textContent=open;e.questionsBadge.classList.toggle('hidden',!open)}wireQuestionActions()}
