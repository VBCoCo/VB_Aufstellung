(() => {
"use strict";
const VERSION="3.0.4.8",DATA_SCHEMA=7,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function isViewerOnly(){return canView()&&!canEdit()}
...
function paths(){e.movementLayer.innerHTML="";e.ballPathLayer.innerHTML="";if((editing&&!playing)||tacticMode||state.step===0)return;const a=rd().steps[state.step-1],b=sd();allPlayers.forEach(p=>{const A=a.positions[p.id],B=b.positions[p.id];if(A&&B&&(A.x!==B.x||A.y!==B.y))line(e.movementLayer,A,B,"movement-path")});if(a.ball.x!==b.ball.x||a.ball.y!==b.ball.y)drawBallCurve(e.ballPathLayer,a.ball,b.ball,"ball-path",false,motionFor(a))}
...
function renderSituationOptions(){
  ...
  e.rotationSelect.disabled=false;e.rotationSelect.value=current;e.situationNameInput.value=situationDisplayName(rd());applySelectTextFit(e.rotationSelect,situationDisplayName(rd()));
...
function renderStepStrip(){
  ...
  rotation.steps.forEach((step,i)=>{
    ...
    b.innerHTML=`<span class="step-strip-label">${i+1} · ${esc(step.name||`Schritt ${i+1}`)}</span>${(step.info||"").trim()?`<span class="step-info-badge" title="Info zu Schritt ${i+1}">ⓘ</span>`:""}`;
...
async function playAll(){
  ...
  stop();activeQuestionId=null;playing=true;state.step=0;render();e.playButton.textContent="■";
...
e.questionsButton?.addEventListener('click',openQuestions);e.courtQuestionButton?.addEventListener('click',()=>{e.questionComposer?.classList.remove('hidden');openQuestions(true)});e.closeQuestions?.addEventListener('click',()=>e.questionsDialog.close());e.questionNewButton?.addEventListener('click',()=>e.questionComposer.classList.toggle('hidden'));e.questionCancel?.addEventListener('click',()=>{e.questionComposer.classList.add('hidden');e.questionText.value=''});e.questionSend?.addEventListener('click',async()=>{const body=e.questionText.value.trim();if(!body)return;e.questionSend.disabled=true;try{await rpc('vt_create_question',{p_team_id:currentTeamId,p_situation_index:state.rotation,p_step_index:state.step,p_situation_name:rname(state.rotation),p_step_name:sd().name,p_body:body});e.questionText.value='';e.questionComposer.classList.add('hidden');await loadQuestions()}catch(err){alert(err.message)}finally{e.questionSend.disabled=false}});e.situationInfoButton?.addEventListener('click',()=>openContextInfo('situation'));e.situationInfoInline?.addEventListener('click',()=>openContextInfo('situation'));e.stepInfoButton?.addEventListener('click',()=>openContextInfo('step'));e.closeContextInfo?.addEventListener('click',()=>e.contextInfoDialog.close());e.situationInfoEdit?.addEventListener('input',()=>{if(editing){rd().info=e.situationInfoEdit.value;dirty=true}});e.stepInfoEdit?.addEventListener('input',()=>{if(editing){sd().info=e.stepInfoEdit.value;dirty=true}});e.publishSituation?.addEventListener('click',async()=>{if(!editing)return;const issues=ruleIssues(rd());if(issues.some(x=>x.severity==='error')){alert('Freigabe nicht möglich: Diese Spielsituation enthält noch rote Regelfehler.');return}if(!rd().published&&(issues.some(x=>x.severity==='warning')||rd().steps.some((s,i)=>stepNeedsWork(s,i,rd())))&&!confirm('Diese Spielsituation enthält noch gelbe Hinweise oder unvollständige Schritte. Trotzdem freigeben?'))return;rd().published=!rd().published;dirty=true;await save(rd().published?'Spielsituation für Viewer freigegeben.':'Freigabe zurückgenommen.');render()});
...
let questionCache=[],activeQuestionId=null;
async function loadQuestions(){if(!navigator.onLine||!currentTeamId)return[];questionCache=await rpc("vt_list_questions",{p_team_id:currentTeamId})||[];renderQuestions();renderQuestionContext();updateCourtQuestionBadge();return questionCache}
function unreadMessageCount(){return questionCache.reduce((sum,q)=>sum+Math.max(0,Number(q.unread_messages)||0),0)}
function refreshQuestionTopBadge(){const unread=unreadMessageCount();if(e.questionsBadge){e.questionsBadge.textContent=unread;e.questionsBadge.classList.toggle('hidden',!unread);e.questionsButton?.classList.toggle('has-unread',unread>0)}}
async function markQuestionRead(id){const q=questionCache.find(x=>x.id===id);if(!q||!(Number(q.unread_messages)>0))return;try{await rpc('vt_mark_question_read',{p_question_id:id});q.unread_messages=0;refreshQuestionTopBadge()}catch(err){console.warn('Frage konnte nicht als gelesen markiert werden',err)}}
function questionStatusLabel(s){return s==='resolved'?'Erledigt':s==='answered'?'Beantwortet':'Offen'}
function questionGroups(){return [['open','Neu / Offen'],['answered','Beantwortet'],['resolved','Erledigt']].map(([status,title])=>{const rows=questionCache.filter(q=>q.status===status);return `<details class="question-group" ${status==='open'?'open':''}><summary>${title}<span>${rows.length}</span></summary><div>${rows.map(questionCard).join('')||'<p class="hint">Keine Einträge.</p>'}</div></details>`}).join('')}
function questionDeleteButton(q){return canEdit()?`<button class="question-delete danger" type="button" data-question-id="${esc(q.id)}" title="Frage löschen">🗑 Löschen</button>`:''}
function questionCard(q){const msgs=q.messages||[],last=msgs[msgs.length-1],preview=(last?.body||q.question||'').slice(0,90),unread=Math.max(0,Number(q.unread_messages)||0);return `<details class="question-card ${unread?'has-unread':''}" data-question-id="${esc(q.id)}"><summary><div><strong>${esc(q.author_name||'Viewer')}</strong><span>${esc(q.situation_name)} · Schritt ${Number(q.step_index)+1}</span><small>${esc(preview)}${preview.length>=90?' …':''}</small></div><div class="question-summary-state">${unread?`<span class="question-unread">${unread} neu</span>`:''}<b>${questionStatusLabel(q.status)}</b></div></summary><div class="question-thread">${msgs.map(m=>`<div class="question-message ${m.author_is_editor?'editor':'viewer'}"><strong>${esc(m.author_name|| (m.author_is_editor?'Bearbeiter':'Viewer'))}</strong><p>${esc(m.body)}</p><small>${new Date(m.created_at).toLocaleString('de-DE')}</small>${editing&&canEdit()?`<button class="question-as-info" data-message-id="${esc(m.id)}" data-question-id="${esc(q.id)}" type="button">ⓘ Als Info übernehmen</button>`:''}</div>`).join('')}<div class="question-context"><span>${esc(q.situation_name)} · Schritt ${Number(q.step_index)+1} · ${esc(q.step_name||'')}</span><button class="question-jump" data-question-id="${esc(q.id)}" data-situation="${q.situation_index}" data-step="${q.step_index}" type="button">Zur Situation</button></div>${q.status!=='resolved'?`<textarea class="question-reply" rows="3" maxlength="2000" placeholder="${canEdit()?'Antwort schreiben …':'Rückfrage schreiben …'}"></textarea><div class="question-actions"><button class="question-reply-send primary" type="button">Senden</button>${canEdit()?'<button class="question-resolve ghost" type="button">Als erledigt markieren</button>':''}${questionDeleteButton(q)}</div>`:`<div class="question-actions"><p class="hint">Dieser Thread ist erledigt.</p>${questionDeleteButton(q)}</div>`}</div></details>`}
function renderQuestions(){if(!e.questionsContent)return;e.questionsContent.innerHTML=questionGroups();refreshQuestionTopBadge();wireQuestionActions()}
async function deleteQuestion(id){const q=questionCache.find(x=>x.id===id);if(!q||!canEdit())return;if(!confirm(`Frage von ${q.author_name||'Viewer'} zu ${q.situation_name} · Schritt ${Number(q.step_index)+1} wirklich löschen?\n\nDer komplette Verlauf wird dauerhaft gelöscht.`))return;await rpc('vt_delete_question',{p_question_id:id});if(activeQuestionId===id)activeQuestionId=null;await loadQuestions()}
function wireQuestionActions(root=e.questionsContent){root?.querySelectorAll('.question-card').forEach(card=>card.addEventListener('toggle',()=>{if(card.open)markQuestionRead(card.dataset.questionId)}));root?.querySelectorAll('.question-jump').forEach(b=>b.addEventListener('click',()=>{markQuestionRead(b.dataset.questionId);const target=Math.max(0,Math.min(td().rotations.length-1,Number(b.dataset.situation)));if(!editing&&!td().rotations[target]?.published){alert(canEdit()?'Diese Spielsituation ist aktuell nicht für Viewer freigegeben. Öffne sie im Bearbeiten-Modus.':'Diese Spielsituation ist aktuell nicht freigegeben.');return}activeQuestionId=b.dataset.questionId;state.rotation=target;state.step=Math.max(0,Math.min(td().rotations[target].steps.length-1,Number(b.dataset.step)));e.questionsDialog?.close();render();e.status.textContent='Frage und Spielsituation geöffnet.'}));root?.querySelectorAll('.question-reply-send').forEach(b=>b.addEventListener('click',async()=>{const card=b.closest('[data-question-id]'),ta=card.querySelector('.question-reply'),body=ta.value.trim();if(!body)return;b.disabled=true;try{await rpc('vt_add_question_message',{p_question_id:card.dataset.questionId,p_body:body});activeQuestionId=card.dataset.questionId;await loadQuestions()}catch(err){alert(err.message)}finally{b.disabled=false}}));root?.querySelectorAll('.question-resolve').forEach(b=>b.addEventListener('click',async()=>{const id=b.closest('[data-question-id]').dataset.questionId;await rpc('vt_set_question_status',{p_question_id:id,p_status:'resolved'});activeQuestionId=id;await loadQuestions()}));root?.querySelectorAll('.question-delete').forEach(b=>b.addEventListener('click',async()=>{try{await deleteQuestion(b.dataset.questionId)}catch(err){alert(err.message)}}));root?.querySelectorAll('.question-as-info').forEach(b=>b.addEventListener('click',()=>{const q=questionCache.find(x=>x.id===b.dataset.questionId),m=q?.messages?.find(x=>x.id===b.dataset.messageId);if(!q||!m)return;const target=confirm('OK = als Info zu diesem Schritt übernehmen.\nAbbrechen = als allgemeine Info zur Spielsituation übernehmen.')?'step':'situation';state.rotation=Number(q.situation_index);state.step=Number(q.step_index);if(target==='step')sd().info=m.body;else rd().info=m.body;dirty=true;activeQuestionId=q.id;e.questionsDialog?.close();edit(true);render();e.status.textContent='Antwort als Info übernommen. Bitte speichern.'}))}
...
function renderQuestionContext(){if(!e.questionContextPanel)return;if(playing){e.questionContextPanel.classList.add('hidden');e.questionContextPanel.innerHTML='';return}const q=questionCache.find(x=>x.id===activeQuestionId);if(!q||Number(q.situation_index)!==state.rotation||Number(q.step_index)!==state.step){e.questionContextPanel.classList.add('hidden');e.questionContextPanel.innerHTML='';return}const msgs=q.messages||[];markQuestionRead(q.id);e.questionContextPanel.classList.remove('hidden');e.questionContextPanel.innerHTML=`<div class="question-context-head"><div><span class="eyebrow">Frage zu diesem Schritt</span><strong>${esc(q.author_name||'Viewer')} · ${questionStatusLabel(q.status)}</strong></div><button class="question-context-close" type="button" aria-label="Frage ausblenden">✕</button></div><div class="question-context-thread" data-question-id="${esc(q.id)}">${msgs.map(m=>`<div class="question-message ${m.author_is_editor?'editor':'viewer'}"><strong>${esc(m.author_name||'Benutzer')}</strong><p>${esc(m.body)}</p><small>${new Date(m.created_at).toLocaleString('de-DE')}</small>${editing&&canEdit()?`<button class="question-as-info" data-message-id="${esc(m.id)}" data-question-id="${esc(q.id)}" type="button">ⓘ Als Info übernehmen</button>`:''}</div>`).join('')}${q.status!=='resolved'?`<textarea class="question-reply" rows="3" maxlength="2000" placeholder="${canEdit()?'Antwort direkt hier schreiben …':'Rückfrage direkt hier schreiben …'}"></textarea><div class="question-actions"><button class="question-reply-send primary" type="button">Senden</button>${canEdit()?'<button class="question-resolve ghost" type="button">Erledigt</button>':''}${questionDeleteButton(q)}</div>`:`<div class="question-actions"><span class="hint">Thread erledigt.</span>${questionDeleteButton(q)}</div>`}</div>`;e.questionContextPanel.querySelector('.question-context-close')?.addEventListener('click',()=>{activeQuestionId=null;renderQuestionContext()});wireQuestionActions(e.questionContextPanel)}
async function openQuestions(startComposer=false){e.questionsDialog.showModal();if(startComposer){e.questionComposer?.classList.remove('hidden');setTimeout(()=>e.questionText?.focus(),50)}e.questionsContent.innerHTML='<p class="hint">Fragen werden geladen …</p>';try{await loadQuestions()}catch(err){e.questionsContent.innerHTML=`<p class="admin-error">${esc(err.message)}</p>`}}
