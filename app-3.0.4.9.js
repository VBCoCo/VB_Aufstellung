(() => {
"use strict";
const VERSION="3.0.4.9",DATA_SCHEMA=7,KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
function copyPlayerPositionsFromPrevious(ids=allPlayers.map(p=>p.id)){
  ...
  const previous=rd().steps[state.step-1];
...
let questionCache=[],activeQuestionId=null,questionRefreshBusy=false;
...
function refreshQuestionsInBackground(){if(document.visibilityState==='hidden'||!authSession||!currentTeamId||!navigator.onLine||e.questionsDialog?.open)return;loadQuestions({silent:true}).catch(()=>{})}
...
function renderQuestionContext(){if(!e.questionContextPanel)return;if(playing){e.questionContextPanel.classList.add('hidden');e.questionContextPanel.innerHTML='';return}const q=questionCache.find(x=>x.id===activeQuestionId);if(!q||Number(q.situation_index)!==state.rotation||Number(q.step_index)!==state.step){e.questionContextPanel.classList.add('hidden');e.questionContextPanel.innerHTML='';return}const msgs=q.messages||[],viewerMode=!editing;markQuestionRead(q.id);e.questionContextPanel.classList.remove('hidden');const asker=viewerMode?'Deine Frage':(q.author_name||'Viewer');e.questionContextPanel.innerHTML=`<div class="question-context-head"><div><span class="eyebrow">Frage zu diesem Schritt</span><strong>${esc(asker)} · ${questionStatusLabel(q.status)}</strong></div><button class="question-context-close" type="button" aria-label="Frage ausblenden">✕</button></div><div class="question-context-thread" data-question-id="${esc(q.id)}">${msgs.map(m=>{const who=viewerMode?(m.author_is_editor?'Bearbeiter':'Du'):(m.author_name||'Benutzer');return `<div class="question-message ${m.author_is_editor?'editor':'viewer'}"><strong>${esc(who)}</strong><p>${esc(m.body)}</p><small>${new Date(m.created_at).toLocaleString('de-DE')}</small>${editing&&canEdit()?`<button class="question-as-info" data-message-id="${esc(m.id)}" data-question-id="${esc(q.id)}" type="button">ⓘ Als Info übernehmen</button>`:''}</div>`}).join('')}${q.status!=='resolved'?`<textarea class="question-reply" rows="3" maxlength="2000" placeholder="${canEdit()?'Antwort direkt hier schreiben …':'Rückfrage direkt hier schreiben …'}"></textarea><div class="question-actions"><button class="question-reply-send primary" type="button">Senden</button>${canEdit()?'<button class="question-resolve ghost" type="button">Erledigt</button>':''}${questionDeleteButton(q)}</div>`:`<div class="question-actions"><span class="hint">Thread erledigt.</span>${questionDeleteButton(q)}</div>`}</div>`;e.questionContextPanel.querySelector('.question-context-close')?.addEventListener('click',()=>{activeQuestionId=null;renderQuestionContext()});wireQuestionActions(e.questionContextPanel)}
