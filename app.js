(() => {
"use strict";
const VERSION="3.13.0",DATA_SCHEMA=8,KEY="volleyball-trainer-v2-2";
...
let positionMenuOpen=false,myPositionEnabled=false,myPositionPlayerId="";
...
let loadedPositionPreferenceKey="";
function positionPreferenceKey(){return `volleyball-trainer-my-position:${authSession?.user?.id||"viewer"}:${currentTeamId||"local"}:${state.teamIndex}`}
function ensurePositionPreference(){const key=positionPreferenceKey();if(key===loadedPositionPreferenceKey)return;loadedPositionPreferenceKey=key;try{const saved=JSON.parse(localStorage.getItem(key)||"{}");myPositionEnabled=Boolean(saved.enabled);myPositionPlayerId=ownRoster.some(p=>p.id===saved.playerId)?saved.playerId:""}catch{myPositionEnabled=false;myPositionPlayerId=""}}
function savePositionPreference(){localStorage.setItem(positionPreferenceKey(),JSON.stringify({enabled:myPositionEnabled,playerId:myPositionPlayerId}))}
function positionChoiceLabel(player){const role=roleNames[ownRole(player)]||ownRole(player),zone=[2,3,4].includes(player.base)?"vorne":"hinten";return `${role} ${zone} · Startposition ${player.base}`}
function highlightedPlayerId(){return !editing&&myPositionEnabled&&ownRoster.some(p=>p.id===myPositionPlayerId)?myPositionPlayerId:""}
function selectedPlayerIsLibero(){const player=ownRoster.find(p=>p.id===highlightedPlayerId());return Boolean(player&&ownRole(player)==="MB"&&td().teamConfig.libero&&[1,5,6].includes(prot(player)))}
function renderPositionControls(){
  ensurePositionPreference();
  if(e.positionInfoMenu)e.positionInfoMenu.classList.toggle("hidden",!positionMenuOpen||editing||tacticMode);
  if(e.positionInfoToggle){e.positionInfoToggle.textContent="Positionen";e.positionInfoToggle.setAttribute("aria-expanded",String(positionMenuOpen&&!editing&&!tacticMode))}
  if(e.positionNumbersEnabled)e.positionNumbersEnabled.checked=showPositionInfo;
  if(e.myPositionEnabled)e.myPositionEnabled.checked=myPositionEnabled;
  if(e.myPositionSelect){const current=myPositionPlayerId;e.myPositionSelect.innerHTML='<option value="">Keine Position</option>'+ownRoster.map(p=>`<option value="${p.id}">${esc(positionChoiceLabel(p))}</option>`).join("");e.myPositionSelect.value=current;e.myPositionSelect.disabled=!myPositionEnabled}
  const libero=selectedPlayerIsLibero();
  if(e.positionViewerNotice){e.positionViewerNotice.classList.toggle("hidden",!libero);e.positionViewerNotice.textContent=libero?"Deine Position wurde durch den Libero ersetzt. Der Libero und seine Laufwege sind markiert.":""}
}
...
function render25(){
  ...
  e.view2d.classList.toggle("active",preferredView==="2d");e.view25d.classList.toggle("active",preferredView==="25d");e.view2d.setAttribute("aria-pressed",String(preferredView==="2d"));e.view25d.setAttribute("aria-pressed",String(preferredView==="25d"));if(e.positionInfoToggle)e.positionInfoToggle.classList.toggle("active",showPositionInfo||myPositionEnabled)
  if(!visible)return;
  ...
  if(state.step>0){const a=rd().steps[state.step-1],b=sd(),highlighted=highlightedPlayerId();allPlayers.forEach(p=>{const A=a.positions[p.id],B=b.positions[p.id];if(A&&B&&(A.x!==B.x||A.y!==B.y))line25(e.movement25Layer,A,B,`movement25-path${p.id===highlighted?" my-position-path":""}`)});if(a.ball.x!==b.ball.x||a.ball.y!==b.ball.y)drawBallCurve(e.ballPath25Layer,a.ball,b.ball,"ball25-path",true,motionFor(a),a,b)}
}
...
function setEditorWorkspace(name){
  ...
  if(name!=="playlists")window.VBMusicLibrary?.close?.();
...
function changeTacticContext({teamIndex=state.teamIndex,rotation=state.rotation}={}){
  if(!tacticMode||!tacticSourceState)return false;
  if(tacticChanged&&!window.confirm("Die freien Änderungen auf der Taktiktafel werden beim Wechsel verworfen. Trotzdem wechseln?")){render();return true}
  tacticSourceState.teamIndex=Math.max(0,Math.min(tacticSourceState.teams.length-1,teamIndex));
  tacticSourceState.rotation=Math.max(0,Math.min(tacticSourceState.teams[tacticSourceState.teamIndex].rotations.length-1,rotation));
  tacticSourceState.step=0;state=structuredClone(tacticSourceState);resetTacticWorkspace();render();return true
}
...
async function animate(target,{sequence=false}={}){
  ...
  let pa,ba;
  if(currentCourtIs25()){
    e.movement25Layer.innerHTML="";e.ballPath25Layer.innerHTML="";allPlayers.forEach(p=>{const A=a.positions[p.id],B=b.positions[p.id];if(A&&B&&(A.x!==B.x||A.y!==B.y))line25(e.movement25Layer,A,B,`movement25-path${p.id===highlightedPlayerId()?" my-position-path":""}`)});if(a.ball.x!==b.ball.x||a.ball.y!==b.ball.y)drawBallCurve(e.ballPath25Layer,a.ball,b.ball,"ball25-path",true,motion,a,b);
...
document.addEventListener("vb-music-library-open-request",()=>{if(editing)setEditorWorkspace("playlists")});
