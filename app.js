(() => {
"use strict";
const VERSION="2.5.2",KEY="volleyball-trainer-v2-2",TEST_PASSWORD="";
...
const initialStep=r=>{const positions={};ownRoster.forEach(p=>positions[p.id]={...ownSlots[rot(p.base,r)]});opponentRoster.forEach(p=>positions[p.id]={...opponentSlots[p.base]});return{name:"Grundposition",situation:"serveReceive",positions,ball:{x:450,y:650},action:{type:"",actorId:"",helperId:""}}};
...
function actionData(){const a=sd().action||{};return{type:a.type||"",actorId:a.actorId||"",helperId:a.helperId||""}}
...
function renderActionEditor(){
  if(!e.actionType)return;const a=actionData();populatePlayerSelect(e.actionActor);populatePlayerSelect(e.actionHelper,{label:"– kein zweiter Spieler –"});
  e.actionType.value=a.type;e.actionActor.value=a.actorId;e.actionHelper.value=a.helperId;e.actionHelperWrap.classList.toggle("hidden",a.type!=="block");
  const actor=allPlayers.find(p=>p.id===a.actorId),helper=allPlayers.find(p=>p.id===a.helperId);
  let txt=a.type?`${actionNames[a.type]||a.type}`:"Kein Ballkontakt definiert";if(actor)txt+=` · ${playerLabel(actor)}`;if(helper)txt+=` + ${playerLabel(helper)}`;
  const next=rd().steps[state.step+1];
  e.actionHint.textContent=a.type?(next?`Dieser Kontakt gehört zu Schritt ${state.step+1}. Beim Übergang zu Schritt ${state.step+2} fliegt der Ball zum dort gespeicherten Ballpunkt und alle Spieler bewegen sich gleichzeitig.`:"Letzter Schritt: Kontakt gespeichert. Für einen weiteren Ballflug einen neuen Schritt anlegen."):"Optional: Kontaktart und ausführenden Spieler festlegen. Der Ballflug ergibt sich automatisch aus dem nächsten Schritt.";
...
function playerPoint(p,player){if(sd().situation==="serveReceive"){const yMin=player.team==="own"?479:79,yMax=player.team==="own"?821:421;return{x:clamp(p.x,124,576),y:clamp(p.y,yMin,yMax)}}return{x:clamp(p.x,32,668),y:clamp(p.y,32,868)}}
...
e.actionActor.addEventListener("change",()=>{sd().action={...actionData(),actorId:e.actionActor.value};syncContactBall();dirty=true;render()});
...
e.clearAction.addEventListener("click",()=>{sd().action={type:"",actorId:"",helperId:""};dirty=true;render()});
