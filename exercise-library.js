(() => {
  ...
  const state = {
    ...
    catalog: [],
    favorites: new Set(),
    density: localStorage.getItem(KEY) || "normal",
    filter: "",
    focus: "",
    difficulty: "",
    favoritesOnly: false,
  ...
  async function context() {
    ...
    const teamId = localStorage.getItem("volleyball-trainer-team-v3") || "";
    const [user, teams] = await Promise.all([API().request("/auth/v1/user"), teamId ? API().request(`/rest/v1/vt_teams?id=eq.${encodeURIComponent(teamId)}&select=id,club_id,name`) : Promise.resolve([])]),
      team = teams?.[0] || null;
    let access = null;
    try {
      access = JSON.parse(localStorage.getItem("volleyball-trainer-access-v3") || "null");
    } catch {}
    const club = (access?.clubs || []).find((x) => x.id === team?.club_id);
    state.context = {
      userId: user.id,
      teamId: team?.id || null,
  ...
  function cachedPlatformAdmin() {
    try {
      return !!JSON.parse(localStorage.getItem("volleyball-trainer-access-v3") || "null")?.platform_admin;
    } catch {
      return false;
    }
  }
  async function featureAccess() {
    try {
      if (cachedPlatformAdmin()) {
        state.access = { allowed: true, reason: "platform_admin" };
        return true;
      }
      const c = await context();
      if (!c.clubId) return false;
      const a = await API().request("/rest/v1/rpc/get_my_feature_access", {
        method: "POST",
        body: { p_feature_key: FEATURE, p_club_id: c.clubId },
      });
      state.access = a;
      return !!a?.allowed;
    } catch (e) {
      console.warn("Exercise beta hidden:", e);
      return false;
    }
  }
  async function inject() {
    const hub = document.getElementById("editorHub"),
      court = document.querySelector(".court-section");
    if (!hub || !court || document.getElementById("editorTileAthletics")) return;
    if (!(await featureAccess())) return;
    const tiles = [
      ["editorTileAthletics", "🏃", "Athletikübungen", "Warm-up, Kraft, Mobilität & mehr"],
      ["editorTileVolleyballExercises", "🏐", "Volleyballübungen", "Übungsformen & Spielformen"],
    ];
    tiles.forEach(([id, icon, title, sub]) => {
      const b = document.createElement("button");
      b.id = id;
      b.className = "editor-tile";
      b.type = "button";
      b.innerHTML = `<span>${icon}</span><strong>${title}</strong><small>${sub}</small>`;
      hub.appendChild(b);
    });
    const section = document.createElement("section");
    section.id = "exerciseLibraryWorkspace";
    section.className = "exercise-library-workspace hidden";
    section.innerHTML = `<div class="exercise-head"><button id="exerciseBack" type="button" aria-label="Zurück zu Bereiche" title="Zurück zu Bereiche">←</button><div><h2 id="exerciseTitle">Athletikübungen</h2></div><div class="exercise-density" aria-label="Anzeigegröße"><button data-density="small" title="Klein">S</button><button data-density="normal" title="Mittel">M</button><button data-density="large" title="Groß">L</button></div></div><div class="exercise-searchbar"><input id="exerciseSearch" type="search" placeholder="Übungen durchsuchen …"><button id="exerciseFilterToggle" type="button">Filter ▾</button><button id="exerciseNew" class="primary" type="button">＋ Neu</button></div><div id="exerciseToolbar" class="exercise-toolbar"><select id="exerciseFocus"><option value="">Alle Schwerpunkte</option></select><select id="exerciseDifficulty"><option value="">Alle Schwierigkeiten</option><option value="easy">Leicht</option><option value="medium">Mittel</option><option value="hard">Schwer</option></select><label class="exercise-check"><input id="exerciseFavoritesOnly" type="checkbox"> Nur Favoriten</label></div><p id="exerciseStatus" class="exercise-status"></p><div id="exerciseList" class="exercise-list"></div>`;
    court.appendChild(section);
    document.getElementById("editorTileAthletics").onclick = () => open("athletics");
    document.getElementById("editorTileVolleyballExercises").onclick = () => open("volleyball");
    document.getElementById("exerciseBack").onclick = close;
    document.getElementById("exerciseFilterToggle").onclick = () => {
      const t = document.getElementById("exerciseToolbar"),
        on = t.classList.toggle("open");
      document.getElementById("exerciseFilterToggle").textContent = on ? "Filter ▴" : "Filter ▾";
    };
    document.getElementById("exerciseSearch").oninput = (e) => {
      state.filter = e.target.value;
      render();
    };
    document.getElementById("exerciseFocus").onchange = (e) => {
      state.focus = e.target.value;
      render();
    };
    document.getElementById("exerciseDifficulty").onchange = (e) => {
      state.difficulty = e.target.value;
      render();
    };
    document.getElementById("exerciseFavoritesOnly").onchange = (e) => {
      state.favoritesOnly = e.target.checked;
      render();
    };
    document.querySelectorAll(".exercise-density button").forEach(
      (b) =>
        (b.onclick = () => {
          state.density = b.dataset.density;
          localStorage.setItem(KEY, state.density);
          render();
        }),
    );
    document.getElementById("exerciseNew").onclick = () => editExercise(null, false);
    document.getElementById("exerciseList").onclick = onListClick;
  }
  async function open(type) {
    state.type = type;
    state.filter = state.focus = state.difficulty = "";
    state.favoritesOnly = false;
    document.body.classList.remove("editor-workspace-hub");
    document.body.classList.add("exercise-library-open");
    document.getElementById("editorHub").classList.add("hidden");
    document.getElementById("exerciseLibraryWorkspace").classList.remove("hidden");
    document.getElementById("exerciseTitle").textContent = type === "athletics" ? "Athletikübungen" : "Volleyballübungen";
    document.getElementById("exerciseSearch").value = "";
    document.getElementById("exerciseDifficulty").value = "";
    document.getElementById("exerciseFavoritesOnly").checked = false;
    await load();
  }
  function close() {
  ...
  async function load() {
    setStatus("Übungen werden geladen …");
    try {
      await context();
      const [items, catalog, favs] = await Promise.all([API().request(`/rest/v1/vt_exercises?exercise_type=eq.${state.type}&select=*&order=name.asc`), API().request("/rest/v1/vt_exercise_catalog_items?active=eq.true&select=kind,code,label,category,parent_code,sort_order&order=sort_order.asc"), API().request("/rest/v1/vt_exercise_favorites?select=exercise_id")]);
      state.items = items || [];
      state.catalog = catalog || [];
      state.favorites = new Set((favs || []).map((x) => x.exercise_id));
      fillFocus();
      render();
    } catch (e) {
  ...
  function fillFocus() {
    const kind = state.type === "athletics" ? "athletics_focus" : "volleyball_focus",
      s = document.getElementById("exerciseFocus");
    s.innerHTML =
  ...
  function playerText(x) {
    ...
    if (x.player_mode === "multiple") return `${x.group_size || "–"}er-Gruppen`;
    if (x.player_mode === "range") return x.player_max ? `${x.player_min || "–"}–${x.player_max} Spieler` : `ab ${x.player_min || "–"} Spieler`;
    return "";
  }
  function materialText(x) {
    const a = Array.isArray(x.materials) ? x.materials : [];
    return a.map((m) => `${label(m.code)}${m.quantity ? ` (${m.quantity})` : ""}`).join(", ") || "Kein Material angegeben";
  }
  function filtered() {
    const q = state.filter.trim().toLowerCase();
    return state.items.filter((x) => (!state.focus || x.main_focus === state.focus) && (!state.difficulty || x.difficulty === state.difficulty) && (!state.favoritesOnly || state.favorites.has(x.id)) && (!q || [x.name, x.short_description, x.goal, label(x.main_focus), ...(x.secondary_tags || []).map(label)].join(" ").toLowerCase().includes(q)));
  }
  function canEditExercise(x) {
    const c = state.context;
    return !!(x?.club_id && c && (x.owner_id === c.userId || c.roles.includes("club_admin")));
  }
  function render() {
    const list = document.getElementById("exerciseList"),
      ws = document.getElementById("exerciseLibraryWorkspace");
    if (!list) return;
    list.dataset.density = state.density;
    if (ws) ws.dataset.density = state.density;
    document.querySelectorAll(".exercise-density button").forEach((b) => b.classList.toggle("active", b.dataset.density === state.density));
    const rows = filtered();
    list.innerHTML =
      rows
        .map((x) => {
  ...
  async function onListClick(e) {
    ...
    if (b.dataset.action === "favorite") return toggleFavorite(b.dataset.id);
    if (b.dataset.action === "edit") return editExercise(item, false);
    if (b.dataset.action === "view") return editExercise(item, true);
    if (b.dataset.action === "clone")
      return editExercise(
        {
          ...item,
          id: null,
          club_id: null,
          name: `${item.name} – eigene Kopie`,
        },
        false,
        item.id,
      );
    if (b.dataset.action === "diagram")
      return window.VBExerciseDiagramEditor?.open({
        exercise: item,
        readonly: !canEditExercise(item),
  ...
  async function toggleFavorite(id) {
    try {
      if (state.favorites.has(id)) {
        await API().request(`/rest/v1/vt_exercise_favorites?exercise_id=eq.${id}`, { method: "DELETE", headers: { Prefer: "return=minimal" } });
        state.favorites.delete(id);
      } else {
        const c = await context();
        await API().request("/rest/v1/vt_exercise_favorites", {
          method: "POST",
          body: { user_id: c.userId, exercise_id: id },
      ...
      setStatus(`Favorit konnte nicht gespeichert werden: ${e.message}`);
    }
  }
  function editExercise(item, readonly, sourceId = null) {
    const isV = state.type === "volleyball",
      overlay = document.createElement("div"),
      focuses = state.catalog.filter((x) => x.kind === (isV ? "volleyball_focus" : "athletics_focus")),
      materials = state.catalog.filter((x) => x.kind === "material"),
  ...
  async function saveExercise(ev, item, overlay, sourceId = null) {
    ...
    const c = await context(),
      ...
      n = Number(fd.get("players")) || null,
      max = Number(fd.get("players_max")) || null,
      payload = {
        exercise_type: state.type,
        club_id: c.clubId,
        team_id: c.teamId,
        owner_id: c.userId,
        visibility: "team",
        name: fd.get("name").trim(),
        short_description: fd.get("short_description").trim(),
        main_focus: fd.get("main_focus"),
        difficulty: fd.get("difficulty"),
        duration_min: Number(fd.get("duration_min")) || null,
        family_key: fd.get("family_key")?.trim() || null,
        variant_label: fd.get("variant_label")?.trim() || null,
        youth_suitable: fd.get("youth_suitable") === "on",
        playful: fd.get("playful") === "on",
        materials: mats.map((code) => ({ code })),
      };
    if (item?.id) {
    ...
    if (isV)
      Object.assign(payload, {
        ...
        field_need: fd.get("field_need")?.trim() || null,
        competition_oriented: fd.get("competition_oriented") === "on",
        parallel_groups: fd.get("parallel_groups") === "on",
        player_mode: mode === "minimum" ? "range" : mode,
        player_exact: mode === "exact" ? n : null,
        player_min: ["minimum", "range"].includes(mode) ? n : null,
        player_max: mode === "range" ? max : null,
        group_size: mode === "multiple" ? n : null,
      });
    else
      Object.assign(payload, {
        execution: fd.get("execution").trim(),
        social_form: fd.get("social_form"),
  ...
  async function boot() {
    for (let i = 0; i < 80 && !window.VBTrainingApi; i++) await new Promise((r) => setTimeout(r, 250));
    if (!window.VBTrainingApi) return;
    await inject();
  }
